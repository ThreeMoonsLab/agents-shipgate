...
- CLI preflight: run `command -v agents-shipgate`, `agents-shipgate --version`, and `agents-shipgate contract --json`. Continue only when the installed CLI reports `minimum_control_contract_version: {{ minimum_control_contract_version }}`; if it is missing or stale, ask the user to install or upgrade `agents-shipgate`.
