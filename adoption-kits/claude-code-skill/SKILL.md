...
3. For verifier runs, parse `agents-shipgate-reports/verifier.json` first:
   `merge_verdict`, `applicability`, `agent_controller`,
   `can_merge_without_human`, `first_next_action`, and `fix_task`. Then parse
   `agents-shipgate-reports/report.json.release_decision.decision`; it is the
   release gate. `agent-result.json` is a supporting/provisional compact
   projection, not the verifier read order.
