...
1. **Install the tool - pin the version so a stale build can't shadow it.** This flow uses the current verifier and Codex-boundary contracts and requires **contract v5 or newer**; an older copy lingering on `PATH` may lack the command or schema fields this prompt expects. Prefer a **pinned, zero-install** runner that fetches the exact version every time instead of trusting whatever is already on `PATH`. **Pin it into one variable and use that for every step below**, so no single command can fall through to a stale binary:
   ...
   SG=agents-shipgate                # only after the line above confirms contract v5+
   ```
