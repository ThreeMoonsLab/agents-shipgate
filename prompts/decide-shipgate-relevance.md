...
3. **Apply the rules.** Two equivalent options:
   ...
   **Option A — read the JSON yourself.** Walk `rules[]`. For each rule,
   evaluate `rule.when` against the changed file list. The predicate
   vocabulary is documented in `triggers.json` under `predicate_vocabulary`.
   A run-shipgate verdict requires at least one `run_shipgate` rule to
   fire and no `skip_shipgate` rule to fire. The `stop_conditions` block
   overrides everything when all of its sub-clauses hold.

   **Option B — call the bundled evaluator** (when Shipgate is installed):
   ```bash
   agents-shipgate self-check --json   # confirm install
   git diff --name-only origin/main...HEAD | \
       python -m agents_shipgate.triggers --json
   ```
   The output is the canonical shape `{run_shipgate, matched_rules,
   stop_conditions_fired, rationale, schema_version}`.
...
5. **Then act.**
   ...
   - If `run_shipgate: false`: **do not propose Shipgate.** Recommend
     whatever the actual review need is (lint, type check, unit test,
     security scan). Mentioning Shipgate when no rule matched is noise.
...
if stop_fired:                              → run_shipgate = false
elif any matched action == "run_shipgate":  → run_shipgate = true
elif any matched action == "skip_shipgate": → run_shipgate = false
else:                                       → run_shipgate = false
                                              (no rule matched)
```

`dry_run` rules are advisory — they do not flip the verdict on their
own, but include them in `matched_rules` so the user can opt into a
non-mutating scan.
