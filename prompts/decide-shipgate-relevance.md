...
5. **Then act.**
   - If `run_shipgate: true`: run verify preview through a **version-pinned,
     ...
     `uvx agents-shipgate@0.16.0 verify --preview --json` (or
     `pipx run agents-shipgate==0.16.0 ...`). Only fall back to a bare
     `agents-shipgate verify --preview --json` once
     `agents-shipgate contract --json` confirms
     `minimum_control_contract_version: 21` (0.16.0 or newer; `--version`
