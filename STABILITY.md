...
- `scenarios[]` shape: `{id, derived_from, tool, adversarial_goal, expected_control, source_findings}`. `id` is `<scenario_type>__<tool_slug>` (or `<scenario_type>__agent` when not tool-scoped). When two distinct tool names slug to the same value (e.g. `a.b` and `a/b` both produce `a_b`), each colliding `id` gains an `__<hash8>` suffix where `hash8` is the first 8 hex chars of `sha256(tool_name)` — applied only on collision so the common case stays clean. `derived_from` is a sorted list of check IDs. `tool` is the original (non-slugged) tool name or `null`. `source_findings` is a sorted list of references back into `report.json`, each one a `findings[].id`, `findings[].fingerprint`, or — when both are absent on a hand-built or older report — `findings[].check_id` (matching the `id or fingerprint or check_id` fallback chain used elsewhere in the report).
