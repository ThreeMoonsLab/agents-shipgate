...
- **Redacted values.** A value the redactors rewrite is `redacted` and also records a blocking `unsupported` coverage issue. That covers known token shapes (`ghp_…`, `AKIA…`, `xoxb-…`), credential assignments, and `docker://user:password@registry/…` userinfo, which is published as `docker://<redacted>@registry/…`. A commit SHA, a tag, `owner/repo/path@ref` and `docker://image@sha256:<hex>` are not rewritten. Two such values could publish the same text, and a digest of either would be a digest of the credential, so the comparison refuses rather than read a distinct change as equal (#767).
