...
- `findings[].provenance_kind` (v0.14+) — records *how a finding was produced*; independent of `confidence`, which records how *sure* we are. Enum: `static_declaration | ast_extraction | keyword_heuristic | regex_heuristic | policy_pack`. The label classifies the rule's own trigger, not the lineage of the facts it inspects: a finding that checks for a declared manifest field against an AST-extracted Tool is still `static_declaration` because the rule reads declared metadata. `static_declaration` covers manifest/MCP/OpenAPI schema facts (high-trust); `ast_extraction` covers findings from the Python framework extractors (ADK, LangChain, CrewAI), which are subject to extraction error; `keyword_heuristic` covers token-list matches (broad scope, read-only prompts, free-text parameter names); `regex_heuristic` covers regex matches (secrets, prompt injection); `policy_pack` covers findings emitted by externally loaded policy packs. Agents can filter by provenance to triage heuristic noise separately from declarative violations. Required + non-nullable on the wire; the field is Python-Optional only so older v0.12/v0.13 reports loaded by `explain-finding` and minimal synthetic test fixtures keep working — emitted reports always carry a real value.
