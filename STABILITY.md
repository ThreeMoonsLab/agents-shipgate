...
publishes that comparison instead of the setup route.
...
- **`source`.** The file, through `public_host_path`, or a member inside it: `<manifest>#mcpServers`, `<manifest>#hooks`, `<marketplace>#plugins.<name>`. A member is named when its text differs between the sides — added, removed or edited — whatever else read the file, because no reader reads that member. A whole file is named only when no inventory published it as an artifact, the file of a grant or a blocking issue: a file a reader read is an item of its own.
