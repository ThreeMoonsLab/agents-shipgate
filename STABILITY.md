...
- **A consumer counting ⚠ rows, `expands: true` or `expansion_signals`** sees fewer for the narrowings above, one more signal for a `:*` widening, and one more ⚠ row and `allow_rule_added` for a rule moved into `allow` under a removed broader allow rule.
