...
**…and here's the failure mode.** Both cases above are constructed fixtures with
...
- **`agents-shipgate-reports/verifier.json`** — the **authoritative PR/control evidence substrate** (`verifier_schema_version: "0.16"`). A coding agent switches on `control.state`, then reads `authorization`, `merge_verdict` (`mergeable | human_review_required | insufficient_evidence | blocked | unknown`), `can_merge_without_human`, `control.next_action`, and `fix_task` when producing reviewer evidence for an agent-capability PR. Only an accepted signed authorization evaluation may expose an exact reviewed command; the release verdict remains unchanged. Local control comes from `shipgate check --format agent-boundary-json` and `shipgate.agent_boundary_result/v2`. See [`docs/agent-contract-current.md`](docs/agent-contract-current.md) for the field contract.
...
- **`agents-shipgate-reports/capability-delta-attestation.json`** — the same capability delta, published as a standalone [in-toto](https://github.com/in-toto/attestation) attestation so a runtime gateway, policy engine, or dashboard can read it without running Agents Shipgate. Written on committed-tree runs only (a worktree run says why on `verifier.base_notes[]`). Verify one with the stdlib-only [`tools/verify-capability-delta.py`](tools/verify-capability-delta.py). Non-gating. See [`docs/capability-delta-attestation.md`](docs/capability-delta-attestation.md).
...
- **`agents-shipgate contract --json`** — verify the installed CLI's local contract before relying on hard-coded schema or gating assumptions; the permission-scoped agent-control model requires `minimum_control_contract_version: "21"`.
...
- **[`docs/capability-delta-attestation.md`](docs/capability-delta-attestation.md)** — the capability delta published as a standalone in-toto attestation (`agents-shipgate-reports/capability-delta-attestation.json`), so a runtime gateway, policy engine, or dashboard can read *what the agent can do after this change* without running Agents Shipgate. Verify one with the stdlib-only [`tools/verify-capability-delta.py`](tools/verify-capability-delta.py). Non-gating.
...
- **[`docs/manifest-v0.1.json`](docs/manifest-v0.1.json)** + **[`docs/report-schema.v0.43.json`](docs/report-schema.v0.43.json)** + **[`docs/agent-handoff-schema.v8.json`](docs/agent-handoff-schema.v8.json)** + **[`docs/verification-receipt-schema.v1.json`](docs/verification-receipt-schema.v1.json)** + **[`docs/human-authorization-schema.v1.json`](docs/human-authorization-schema.v1.json)** — JSON Schemas for live editor validation, agent routing, reproducible verification identity, and the external signed-authorization family. Reports carry `report_schema_version: "0.43"`; every policy finding records typed predicate support, while heuristic-only, mixed, unknown, or conflicting applicability becomes a non-waivable evidence gap. `passed` still requires a complete binding graph from its entry points and complete identity, effect, authority, and policy evidence for each reachable action. A successful verifier run writes a terminal content-addressed receipt last; validate it before trusting the handoff or report. Every release decision remains static-only. See [`docs/verification-reproducibility.md`](docs/verification-reproducibility.md) and [`docs/passed-verdict-contract.md`](docs/passed-verdict-contract.md). v0.42 is frozen.
- **[`docs/capability-lock-schema.v0.8.json`](docs/capability-lock-schema.v0.8.json)** + **[`docs/capability-lock-diff-schema.v0.9.json`](docs/capability-lock-diff-schema.v0.9.json)** — current capability standard v0.5 schemas (lock `0.8`, diff `0.9`), including binding hashes, for the static capability envelope and semantic diff; non-gating and separate from `report.json`.
- **[`docs/capability-payload-schema.v1.json`](docs/capability-payload-schema.v1.json)** + **[`docs/capability-delta-attestation-schema.v1.json`](docs/capability-delta-attestation-schema.v1.json)** — the frozen interchange pair: one payload with two views, and the in-toto statement that publishes the delta view. Closed schemas, versioned as a whole; non-gating.
...
- [JSON report schema v0.43](docs/report-schema.v0.43.json)
- [JSON report schema v0.40 (frozen)](docs/report-schema.v0.40.json)
- [JSON report schema v0.38](docs/report-schema.v0.38.json)
...
- [Capability lock schema v0.8](docs/capability-lock-schema.v0.8.json)
- [Capability lock diff schema v0.9](docs/capability-lock-diff-schema.v0.9.json)
- [Capability payload schema v1](docs/capability-payload-schema.v1.json)
- [Capability delta attestation schema v1](docs/capability-delta-attestation-schema.v1.json)
