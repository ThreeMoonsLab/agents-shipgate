...
agents-shipgate init --workspace . --write --ci --agent-instructions=all
Before editing trust roots or proposing a high-risk action, run:
...
- **`agents-shipgate preflight --json`** — the proactive, pre-edit planning surface. It lists protected surfaces, forbidden shortcuts, required evidence for proposed high-risk actions, and policy/trust-root hashes. It is a routing aid only, not a release verdict.
