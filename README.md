...
- The manifest remains `version: "0.1"` so existing configs keep working. Current reports carry `report_schema_version: "0.23"` (additive over v0.22's verifier-cycle blocks, adding semantic metadata to `capability_change` members) while preserving the stable payload contract documented in the report schema.
...
- [JSON report schema v0.23](docs/report-schema.v0.23.json)
