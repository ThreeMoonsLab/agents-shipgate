...
- The manifest remains `version: "0.1"` so existing configs keep working. Current reports carry `report_schema_version: "0.38"`; every narrowing decision is recorded in `surface_exclusions` and reachable by the release decision, while v0.37 remains frozen for archived reports.
