...
@pytest.mark.parametrize(
    ...
    [
        ("docs/verifier-schema.v0.20.json", ("control",)),
...
def test_generated_public_schemas_reject_contradictory_control(
    ...
    handoff = build_agent_handoff(verifier=verifier, verify_run=run)
    payload_by_schema = {
        "docs/verifier-schema.v0.20.json": verifier.model_dump(mode="json"),
...
@pytest.mark.parametrize(
    "schema_path",
    ["docs/verifier-schema.v0.20.json", "docs/agent-handoff-schema.v9.json"],
)
...
def test_passed_wrapper_contradictions_fail_pydantic_and_generated_schema(
    ...
    with pytest.raises(ValidationError):
        VerifierArtifact.model_validate(payload)
    schema = json.loads((ROOT / "docs/verifier-schema.v0.20.json").read_text())
    assert list(Draft202012Validator(schema).iter_errors(payload))
