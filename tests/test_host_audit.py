...
def test_inventory_v02_collects_typed_multi_host_grants(tmp_path: Path) -> None:
    ...
    assert inventory["host_grants_inventory_schema_version"] == "0.5"
    HostGrantsInventoryV5.model_validate(inventory)
...
def test_v02_baseline_is_typed_portable_redacted_and_idempotent(tmp_path: Path) -> None:
    ...
    HostGrantsBaselineV5.model_validate(payload)
    assert payload["host_grants_schema_version"] == "0.5"
...
def test_clean_and_changed_v02_drift(tmp_path: Path) -> None:
    ...
    assert code == 0
    HostGrantsDriftV5.model_validate(clean)
...
def test_legacy_v01_baseline_is_incomparable_advisory_and_strict_20(tmp_path: Path) -> None:
    ...
    HostGrantsDriftV5.model_validate(shared)
    ...
    code, payload = _drift_json(tmp_path)
    assert code == 0
    HostGrantsDriftV5.model_validate(payload)
...
def test_generated_models_reject_unknown_fields_and_invalid_literals(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError):
        HostGrantsInventoryV5.model_validate({**payload, "legacy_parse_warnings": []})
    with pytest.raises(ValidationError):
        HostGrantsInventoryV5.model_validate({**payload, "scope": "runtime"})
