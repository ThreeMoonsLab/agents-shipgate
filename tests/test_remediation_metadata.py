...
- The 3 stale-manifest checks override to `autofix_safe=True` /
  `suggested_patch_kind="remove_pointer"`.
- `SHIP-AUTH-SCOPE-COVERAGE-MISSING` overrides
  `suggested_patch_kind="append_pointer"` but stays `autofix_safe=False`
  (default `apply-patches --confidence high` skips it).
- Trace approval/confirmation findings stay at the safe-closed default
  ...
  behavior.
- Every `docs_url` resolves to a real `### SHIP-...` heading anchor in
  `docs/checks.md`.
"""
...
def test_stale_manifest_checks_are_high_confidence_remove_pointer(check_id):
    check = next(c for c in _builtin_checks() if c.id == check_id)
    assert check.autofix_safe is True, (
        f"{check_id} must be auto-applied at default --confidence high"
    )
    assert check.requires_human_review is False
...
def test_regenerated_checks_json_includes_new_fields():
    ...
    assert stale["autofix_safe"] is True
