...
def test_one_capability_cannot_appear_under_two_rows_of_a_subject() -> None:
    ...
    CapabilityDeltaSubject(
        ...
        changes=(
            CapabilityRecordTransitionEntry(
                transition="reidentified",
                changed_dimensions=("identity_hash",),
                before=before,
                after=after,
            ),
        ),
            ...
            changes=tuple(
                sorted(
                    (
                        ...
                        CapabilityRecordTransitionEntry(
                            transition="removed", before=before
                        ),
                    ),
                    key=lambda entry: (
...
def test_transition_and_sides_must_agree() -> None:
    ...
    with pytest.raises(ValidationError, match="forbids a 'before' record"):
        CapabilityRecordTransitionEntry(
            transition="added", before=record, after=record
        )
    with pytest.raises(ValidationError, match="requires an 'after' record"):
        CapabilityRecordTransitionEntry(transition="changed", before=record)
    ...
    with pytest.raises(ValidationError, match="must name at least one changed"):
        CapabilityRecordTransitionEntry(
            transition="changed", before=record, after=record
        )
...
def test_analysis_coverage_defaults_to_not_requested() -> None:
    ...
    assert payload.analysis_coverage.status == "not_requested"
    assert payload.analysis_coverage.subjects_outside_analysis == ()
...
def test_a_subject_outside_analysis_is_carried_and_validates(refund_facts) -> None:
    ...
    delta = project_capability_delta(base, head, analysis_coverage=coverage)
    for payload in (state, delta):
        assert payload.analysis_coverage.subjects_outside_analysis == (unbound,)
        jsonschema.validate(payload.model_dump(mode="json"), PAYLOAD_SCHEMA)
        assert CapabilityPayloadV1.model_validate(
            payload.model_dump(mode="json")
        ).root.analysis_coverage == coverage
...
def test_outside_analysis_and_delta_rows_are_separate_axes(refund_facts) -> None:
    ...
    payload = project_capability_delta(
        ...
        analysis_coverage=CapabilityAnalysisCoverage(
            status="complete",
            subjects_outside_analysis=(gone,),
        ),
    )
    assert payload.analysis_coverage.subjects_outside_analysis == (gone,)
    jsonschema.validate(payload.model_dump(mode="json"), PAYLOAD_SCHEMA)
...
def test_permission_unavailable_cannot_claim_known_side_effects() -> None:
    from agents_shipgate.schemas.capability_payload import CapabilityPermissionFacts

    with pytest.raises(ValidationError, match="not side-effect free"):
        CapabilityPermissionFacts(status="unavailable", side_effect_unknown=False)
