...
def test_script_matches_cli_for_named_mcp_code_idiom(script_module, tmp_path):
    ...
    assert script_result["suggested_sources"] == cli_result["suggested_sources"] == [
        {
            "type": "mcp",
            "path": "server.ts",
    ...
    assert {item["type"] for item in script_result["frameworks"]} == {
        item["type"] for item in cli_result["frameworks"]
    } == {"mcp_server"}


def test_script_matches_cli_for_mcp_source_import_marker(script_module, tmp_path):
    (tmp_path / "package.json").write_text(
...
def test_script_matches_cli_for_dynamic_only_mcp_code_idiom(
    ...
    script_mcp = next(
        item for item in script_result["frameworks"] if item["type"] == "mcp_server"
    )
    cli_mcp = next(
        item for item in cli_result["frameworks"] if item["type"] == "mcp_server"
    )
    assert script_mcp["evidence"] == cli_mcp["evidence"]
    assert any("runtime-built name(s) omitted" in row for row in script_mcp["evidence"])


@pytest.mark.parametrize(
    ("filename", "project_file", "project_text", "source_text", "idiom"),
    [
        (
            "definitions.ts",
            "package.json",
            '{"name":"expression-mcp-server"}',
            'class Dynamic { static toolName = "prefix-" + suffix; }',
            "typescript_static_tool_v1",
        ),
        (
            "server.go",
            "go.mod",
            "module example.com/expression-mcp-server\n",
            """
package main
func mount(server *mcp.Server) {
    mcp.AddTool(server, &mcp.Tool{Name: "prefix-" + suffix}, handler)
}
""",
            "go_addtool_v1",
        ),
...
def test_script_matches_cli_for_other_dynamic_literal_prefix_forms(
    script_module,
    tmp_path,
    filename,
    project_file,
    ...
    (tmp_path / project_file).write_text(project_text, encoding="utf-8")
    (tmp_path / filename).write_text(source_text, encoding="utf-8")

    script_result = script_module.detect(tmp_path)
    cli_result = detect_workspace(tmp_path.resolve()).model_dump(mode="json")

    expected = [{"type": "mcp", "path": filename, "idiom": idiom}]
    assert script_result["suggested_sources"] == cli_result["suggested_sources"] == expected
    script_evidence = next(
        item["evidence"]
        for item in script_result["frameworks"]
        if item["type"] == "mcp_server"
    )
    cli_evidence = next(
        item["evidence"]
        for item in cli_result["frameworks"]
        if item["type"] == "mcp_server"
    )
    assert script_evidence == cli_evidence
    assert any("runtime-built name(s) omitted" in row for row in script_evidence)


def test_script_matches_cli_export_preference_for_mcp_code(
    script_module, tmp_path
):
    (tmp_path / "package.json").write_text(
        '{"name":"preferred-mcp"}', encoding="utf-8"
    )
    (tmp_path / "server.ts").write_text(
        'server.registerTool("lookup", {}, lookup);', encoding="utf-8"
    )
    (tmp_path / "published-mcp.json").write_text(
        '{"tools":[{"name":"lookup"}]}', encoding="utf-8"
    )
...
def test_script_matches_cli_for_distinct_export_and_code_servers(
    script_module, tmp_path
):
    (tmp_path / "package.json").write_text(
...
def test_script_matches_cli_for_github_per_tool_snapshots(script_module, tmp_path):
    ...
    assert script_mcp == cli_mcp


def test_script_matches_cli_when_regex_only_mentions_registration(
    script_module, tmp_path
):
    (tmp_path / "package.json").write_text(
    ...
    assert script_result["is_agent_project"] == cli_result["is_agent_project"] is False
    assert script_result["suggested_sources"] == cli_result["suggested_sources"] == []


def test_script_matches_cli_for_unbalanced_registration(script_module, tmp_path):
    (tmp_path / "package.json").write_text(
        '{"dependencies":{"@modelcontextprotocol/sdk":"^1"}}', encoding="utf-8"
    )
    (tmp_path / "server.ts").write_text(
...
def test_script_matches_cli_for_nested_typescript_templates(
    script_module, tmp_path
):
    (tmp_path / "package.json").write_text(
...
class CreateDBUser {
  ...
  execute(username) {
    return `User "${username}" created${generated ? ` with: \`${password}\`` : ""}.`;
  }
}
''',
        encoding="utf-8",
...
def test_script_matches_cli_for_grafana_runtime_registration_surface(
    script_module, tmp_path
):
    (tmp_path / "go.mod").write_text(
    ...
    tools.mkdir()
    (tools / "definitions.go").write_text(
        "\n".join(
            [
                "package tools",
                *[
    ...
    (tools / "register.go").write_text(
        "\n".join(
            [
                "package tools",
                "func Register(mcp *server.MCPServer) {",
                *[f"Tool{index}.Register(mcp)" for index in range(100)],
                "}",
            ]
        ),
        encoding="utf-8",
...
""",
    ...
    assert script_result["suggested_sources"] == cli_result["suggested_sources"] == [
        {"type": "mcp", "path": "tools/definitions.go", "idiom": "go_musttool_v1"}
    ]
    evidence = next(
        item["evidence"]
        for item in script_result["frameworks"]
    ...
    assert any("100 literal tool registration(s)" in row for row in evidence)


def test_script_matches_cli_mcp_code_aggregate_cap(
    script_module, tmp_path, monkeypatch
):
    from agents_shipgate.cli.discovery import signals

    (tmp_path / "package.json").write_text(
        '{"name":"bounded-mcp"}', encoding="utf-8"
    )
    (tmp_path / "server.ts").write_text(
        'server.registerTool("lookup", {}, lookup);', encoding="utf-8"
    )
    monkeypatch.setattr(script_module, "MAX_MCP_CODE_TOTAL_BYTES", 16)
    ...
    assert script_result["suggested_sources"] == cli_result["suggested_sources"] == []
    assert script_result["excluded_sources"] == cli_result["excluded_sources"]
    assert script_result["next_action"] == cli_result["next_action"]


def test_script_matches_cli_mcp_code_per_file_cap(
    script_module, tmp_path, monkeypatch
):
    from agents_shipgate.cli.discovery import signals
    ...
    monkeypatch.setattr(script_module, "MAX_MCP_CODE_FILE_BYTES", 16)
