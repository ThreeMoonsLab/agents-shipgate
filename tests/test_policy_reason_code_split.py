...
def test_the_frozen_v0_8_schema_still_describes_v0_8_artifacts():
    ...
    current = json.loads(
        (REPO_ROOT / "docs" / "verifier-schema.v0.13.json").read_text("utf-8")
    )
    ...
    assert current["properties"]["verifier_schema_version"]["const"] == "0.13"

    from agents_shipgate.schemas.verifier import VerifierArtifact

    assert VerifierArtifact.model_fields["verifier_schema_version"].default == "0.13"
...
def test_a_v0_8_artifact_still_reads_and_normalizes_forward(tmp_path):
    ...
    assert normalized.verifier_schema_version == "0.13"
