...
- [`report-schema.v0.14.json`](report-schema.v0.14.json) — JSON Schema for `report.json` (current; emitted reports carry `report_schema_version: "0.14"`, which adds the `insufficient_evidence` value to the `release_decision.decision`/`agent_summary.verdict` enums)
