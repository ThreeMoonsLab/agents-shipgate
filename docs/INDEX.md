...
- [`report-schema.v0.15.json`](report-schema.v0.15.json) — JSON Schema for `report.json` (current; emitted reports carry `report_schema_version: "0.15"`, which adds the per-finding `provenance_kind` enum so agents can filter heuristic-only findings from declarative ones)
...
- [`packet-schema.v0.4.json`](packet-schema.v0.4.json) — JSON Schema for the Release Evidence Packet (current; emitted packets carry `packet_schema_version: "0.4"`)
