...
| `shipgate.handoff` | `{verifier_path, report_path?, verify_run_path?}` | exact `shipgate.agent_handoff/v4` |
...
returns the same `shipgate.agent_handoff/v4` shape that `verify` writes for
