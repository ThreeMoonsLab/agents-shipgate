...
| `scope` | `escalate_to_human` | Either remove the offending tool from this release or update `agent.declared_purpose` / `prohibited_actions` to match. Both must be supplied by a human: escalate the choice, never write it yourself. | Don't suppress — the contradiction will surface again |
