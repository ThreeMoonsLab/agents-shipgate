...
sets `blocks_release`, so a heuristic can route attention without ever gating a
release on its own; a repository that configures `fail_on: [medium]` still
applies its own policy to this finding like any other medium one.
