...
Only a variable name assigned in more than one function — two builders' local
`agent` — gives way to each agent's literal `name`, and a handoff to such a
...
- an agent whose `tools`, `handoffs` or `mcp_servers` are changed after
  ...
  the scope of the change: an agent the reader constructed — directly, through
...
an agent without it: a capability-passing copy, and a change to the tools,
handoffs, MCP servers or sub-agents of an object it imports from the scope
(`agent.quote_agent.tools.append(...)`); another library's `.tools` on its own
objects is not read as an agent's. In any module, importing an SDK `Agent`
subclass from another module — `from core import Assistant`, or `core.Assistant`
after `import core` — is a limit; a subclass nothing imports, or a name that
only appears in a string, is not. Each is a limit on the module where it
appears.
