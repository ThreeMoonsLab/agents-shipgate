...
| Adoption benchmark was not a governance acceptance spec | [`../benchmark/agent-pr-governance/`](../benchmark/agent-pr-governance/) defines the AgentPR Governance benchmark and case catalog; executable rows run real verify plus capability-lock diff assertions, while catalog-only rows remain backlog | `tests/test_governance_benchmark.py`; `tests/test_product_hardening_gap_closure.py` validate the executable subset, catalog shape, category coverage, and decision coverage |
...
- Governance benchmark, trace, and runtime-inventory artifacts do not introduce a report schema bump or a second verdict.

## Ongoing Rule

When design-partner feedback exposes a false positive, missed capability, or
unsafe-pass risk, add the smallest reproducible artifact to the governance
benchmark catalog first. Add runtime execution, live inventory, or hosted workflow only
