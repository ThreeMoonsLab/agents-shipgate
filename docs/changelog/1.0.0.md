...
- Re-run the three live measurements on the final engine and commit them as the
  runs of record (#658, #659, #660). #767, #768, #713 and #581 changed engine bytes
  after the `e5ec2311` runs, so each harness ran again on the wheel Release Engine
  Smoke built and exercised on `747d6080`, sha256 `1b846258…`. No outcome moved:
  host-config and the ten MCP servers reproduce `e5ec2311`'s `scores.csv` byte for
  byte (precision 70/70, widening recall 55/65, benign zero-row 5/6; 76 findings,
  1 false), and cold start matches every outcome column at 26 of 30. Each
  `runs.json` was masked with `mask_local_paths` and re-scored byte for byte, and
  the two tests that re-score a run of record now point at these directories.

- Name three more unread host surfaces on the support page, and the link rule
  (#693, #714, #771, #772). An adversarial probe of `shipgate diff` during the
  1.0 readiness review found three edits that give a comparable result with no
  row and no coverage limit: a workflow step's action reference moved from a
  pinned SHA to `@main` (#771), a named secret mapping on a reusable-workflow
  call (#693), and a remote MCP server's URL path, which stays out of the digest
  because a webhook-style path can itself be the secret (#772). The page now
  names all three beside #701 and #702, says that a `.claude/hooks/hooks.json`
  row does not establish that Claude Code loads the file (#714), and states that
  a dangling link, a link leaving the repository, or a directory link outside
  the boundary paths refuses the whole comparison (#688, #659).
  `tests/test_host_boundary_unread_surfaces.py` also runs the page's examples:
  each named surface must still produce no row, and each fixture must produce
  one row when a covered field changes, so a reader fix fails until its entry
  leaves the page.

- Read Git pathnames that contain spaces in `check`, `verify` and the MCP
  `check` tool (#581). Git never quotes a space, but the shared unified-diff
  parser split every unquoted `diff --git` header at its first space and read
  `---`, `+++` and `rename` values the same way. A change touching
  `docs/new scope/notes.md` became a record whose paths were the
  `\0invalid-diff-path` sentinel: `check` published it as a changed file and
  asked for review of a docs-only change, and a scoped manifest under a spaced
  directory crashed `verify` with `lstat: embedded null character`. An unquoted
  header pair is now resolved the way `git apply` resolves it, by the one split
  whose halves name the same file; a rename or copy between spaced names is
  named by its `rename`/`copy` lines, and a record nothing names is refused.
  Path values are taken whole up to the first TAB, which Git never leaves
  unquoted, and `is_configured_manifest` treats a path no filesystem entry can
  have as not the manifest. The header, `rename`/`copy` and `---`/`+++`
  consistency checks are kept. `diff` and `audit --host` never parsed unified
  diffs and were unaffected.

- Build the optional MCP server on the SDK 2.x `MCPServer` (#713). The `[mcp]`
  extra has installed MCP Python SDK 2.x since #354, while the server imported
  `mcp.server.fastmcp.FastMCP`, which SDK 2.0 renamed and replaced with a module
  that raises. `mcp-serve` therefore could not start on any install of the
  declared extra, and its error told the user to install the extra again. No CI
  job installed the extra, so the one test that builds the server was skipped
  everywhere. The server now imports `MCPServer`; a missing SDK and an SDK
  without `MCPServer` are told apart, the second naming the installed version
  and the required range; the floor is `2.0.0`; and a test holds the range the
  server names equal to `pyproject.toml`'s. A new `mcp-extra` CI job installs
  the SDK at that floor and at the newest release the range admits, runs the MCP
  tests against it, and lists the five read-only tools over stdio from the
  installed command.

- Report incomplete coverage for malformed Claude permission shapes (#768).
  `{"permissions":{"allow":"Bash(*)"}}` is valid JSON but not an interpretable
  permission array; the host reader dropped it silently, reported complete
  coverage and produced a comparable zero-row diff, so the generated Stop hook
  stayed quiet. The reader now validates the known containers, an object
  document, an object `permissions` when present, and arrays of non-empty
  strings for `allow`, `deny` and `ask`, before publishing a parsed artifact.
  An invalid input becomes an `unsupported` blocking inventory issue and
  incomplete coverage, with a diagnostic that names the field and expected type
  without echoing values. Missing fields, empty arrays and unknown extension
  keys stay accepted. An unsupported file proven byte-identical keeps #721's
  treatment: `diff` and `verify` name it as a standing limit, and `check`
  refuses because its boundary projection cannot represent one.

- Preserve permission-change semantics through `check`'s redaction (#767). A
  committed `Bash(npm *)` → `Bash(npm test:*)` narrowing was non-expanding in
  `diff` and `verify`, but `check` reported its added row as expanding while its
  control said the settings narrowed: the host comparison redacted the rules
  before recalculating expansion signals, which erased scope containment. Row
  semantics are now computed from the original typed grants and their original
  expansion signals, and only the displayed permission-rule values are redacted
  while rows are built. The input payload stays intact, distinct changes that
  share a redacted label stay separate rows, and the control decision and row
  schema are unchanged.

- Exercise the #659 oracle with controls, and record how its expectations are
  labelled (#659). The host-config harness published rates, but nothing checked
  the oracle and scorer themselves, and its README did not say who labels a case
  or what the labeller had seen. `tests/test_host_config_oracle_controls.py` now
  requires that both oracles and the scorer import nothing from `agents_shipgate`.
  It runs a positive control (an allow rule added), a narrowing control (an allow
  rule removed) and a neutral-inside-config control (the same rules, reordered and
  reformatted). A zero-output implementation fails the positive and narrowing
  controls, a false-widening implementation fails the narrowing and neutral
  controls, and the engine, run through `shipgate diff`, passes all three. The
  README now states that no person labels the cases and no agreement score
  exists. It also names the two amendments made after engine output was seen:
  #734's `extraKnownMarketplaces` and #754's JSON with comments.

- Give release verification's correctness suite a budget re-derived for today's
  suite (#648). Both `release-verify.yml` and `release-advisory-verify.yml` run
  the whole suite on one runner within 20 minutes. That budget came from a 407s
  measurement taken on 2026-08-09, when the suite had 3,099 test functions; it
  now has 6,137, and CI already splits it into three shards (6.6, 11.3 and 6.3
  minutes on hosted runners). The first Advisory Release Rehearsal on the 1.0.0
  candidate timed out 19.7 minutes into the suite, and a tagged release would
  have failed the same way after the tag existed. Following the runbook's
  re-derivation rule, the budget is now 60 minutes, about 2.4x the ~25 the suite
  needs, and the runbook's timing table records the new figures and why.

- Re-run the three live measurements on the 1.0.0 candidate wheel and commit
  them as the runs of record (#658, #659, #660). Each ran on the wheel Release
  Engine Smoke built and exercised on `e5ec2311`, sha256 `071abe4f…`.
  Host-config: row precision 70/70, widening recall 55/65, benign zero-row 5/6;
  the one change is `amelioro/ameliorate#936`, which compares since `.vscode/mcp.json`
  is read as JSON with comments. Cold start: 26 of 30, every stop point
  unchanged. Ten MCP servers: 76 findings, 1 false, identical to the previous
  run. A live run records third-party configuration verbatim, and two public
  repositories commit a home directory into theirs; nothing masked `runs.json`,
  and the earlier runs were masked by hand. Each new run was masked with the
  vendored cases' `mask_local_paths` and re-scored byte for byte, and
  `tests/test_benchmark_results_privacy.py` now refuses a committed run that
  carries an unmasked home or temporary path.

- Name the host surfaces no adapter reads on the support page (#701, #702).
  A composite action a workflow invokes runs inside the calling job, with its
  `permissions` and secrets, and a script a hook command runs changes what the
  hook executes. Neither file is read, so editing either produces no row and no
  coverage limit, and `docs/host-boundary-support.md` did not say so. A
  *Known unread surfaces* section now names both, with the issue that owns each
  reader. Reading them needs a grant-shape decision (#701) and conservative path
  resolution (#702), which stay open.
