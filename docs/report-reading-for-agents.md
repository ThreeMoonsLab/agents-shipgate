...
| `"insufficient_evidence"` (v0.14+) | The scan cannot establish a defensible static pass. In v0.29 this includes any action with unknown, inferred-only, protocol-defaulted, partial, conflicting, invalid, or incomplete required effect/authority evidence, plus the existing extraction-confidence and source-warning thresholds. This does not prove the agent is unsafe. | Read `semantic_coverage` and work each `evidence_gaps[].next_action`, **starting with the first *addressable* row — one whose `next_action.path` names a visible target, or whose `next_action.command` is publishable as authored** — that is the row `reason` and `agent_summary.first_recommended_action` also name (v0.16+), not necessarily `evidence_gaps[0]`. Only when **no row has a visible target and none carries a publishable command** is there nothing to open or run — that is when gathering deeper sources really is the next step. A `path: null` row with a publishable `command` is still addressable and still the row to work. Effect/authority declarations are human assertions: never auto-write them. Do not auto-merge. |
| `"review_required"` | Review items exist (often baseline-matched accepted debt, capability/intent misalignments, or sub-threshold evidence gaps). This **also** covers a degraded-evidence case that carries an active (non-baseline-accepted) high/critical finding — the verdict names the concern instead of the vaguer `insufficient_evidence`, but the evidence gap is still present in `evidence_coverage`. One to three source warnings without blockers also land here. | Surface review items as a human handoff. Safe mechanical patches may still apply via `apply-patches --confidence high` — **unless** evidence is degraded, in which case treat it like `insufficient_evidence`: start with the first *addressable* `evidence_gaps[]` row (visible `next_action.path`, **or** a publishable `next_action.command`), and gather deeper sources only when no row offers either. "Degraded" is not just the two extraction counts: `binding_coverage.gap_count`, `semantic_coverage.gap_count`, `policy_gap_count`, any `source_warning` gap whose `next_action.kind` is `provide_source`, `low_confidence_tool_count` against the ratio threshold, **or** `source_warning_count` over the tolerated maximum all count. On this verdict `reason` stays severity-driven and never names a gap, and `agent_summary.first_recommended_action` names one only on the evidence-first branches — with auto-applicable patches and sub-threshold evidence it is the `apply-patches` command instead. Read `evidence_gaps[]` directly for the evidence step rather than inferring it from either field. `verify`'s `fix_task` already routes degraded-evidence cases to a human. |
...
The packet schema is `0.17`; full schema at [`docs/packet-schema.v0.17.json`](packet-schema.v0.17.json). It projects current report binding and semantic coverage plus gap remediation; v0.16 and older versions are frozen references.
...
|---|---|---|---|
| Report | `0.42` | `0.41`, `0.40`, `0.39`, `0.38`, `0.37`, `0.36`, `0.35`, `0.34`, `0.33`, `0.32`, `0.31`, `0.30`, `0.29`, `0.28`, `0.27`, `0.26`, `0.25`, `0.24`, `0.23`, `0.22`, `0.21`, `0.20`, `0.19`, `0.18`, `0.17`, `0.16`, `0.15`, `0.14`, `0.13`, `0.12`, `0.11`, `0.10`, `0.9`, `0.8`, `0.7`, `0.6`, `0.5`, `0.4`, `0.3`, `0.2`, `0.1` | [`report-schema.v0.42.json`](report-schema.v0.42.json) |
| Packet | `0.17` | `0.16`, `0.15`, `0.14`, `0.13`, `0.12`, `0.11`, `0.10`, `0.9`, `0.8`, `0.7`, `0.6`, `0.5`, `0.4`, `0.3`, `0.2`, `0.1` | [`packet-schema.v0.17.json`](packet-schema.v0.17.json) |
| Manifest | `0.1` | — | [`manifest-v0.1.json`](manifest-v0.1.json) |
| CLI contract | `25` | — | `agents-shipgate contract --json` |
