...
| `shipgate.yaml` still has `CHANGE_ME` | `SHIP-DIAG-CHANGE-ME-PLACEHOLDERS` | `placeholders[]` gives locations only; `control.next_action` does the routing, naming the fields and lines a person must supply. `agent.name` is repository reading a coding agent may do. `agent.declared_purpose` must be supplied by a human — Shipgate never invents a declaration nobody made, so do not derive it from a prompt, main agent file or README. |
