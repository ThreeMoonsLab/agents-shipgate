...
output; `--format pointer` returns the raw pointer). It has a published budget
of `agent_control_max_bytes` (4096) and is never written to disk — the
artifacts it names stay where they are.
...
carries no implication at all. `exit_code` reports the CI gate signal, which is
