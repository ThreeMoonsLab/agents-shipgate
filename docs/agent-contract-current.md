...
- **Evidence-gap actionability (v0.16+).** One **selected gap** feeds every short-form surface that names one: the first `release_decision.evidence_coverage.evidence_gaps[]` row whose `next_action.path` is a non-empty string, falling back to the first row when no row is addressable. `Improve evidence:` (CLI and step summary, printed only on an `insufficient_evidence` verdict) always renders the selected gap's action. Two guarantees, scoped to exactly what holds:
  - **Alignment, on `insufficient_evidence` with an addressable gap.** When the verdict is `insufficient_evidence` *and* at least one gap carries a non-empty `next_action.path`, `release_decision.reason`, `Improve evidence:`, and `first_recommended_action.why` name the **same** gap and the **same** path. The reason leads with that gap and reports the source-warning / low-confidence counts as `Context:`.
  - **No false dead end, on every verdict.** The phrase *"no machine-applicable fix is available"* is never emitted in `first_recommended_action.why` while any gap carries a non-empty `next_action.path`. When you do see it, no gap names a surface to open and the next step really is a human gathering evidence.
  ...
  - `review_required` (including the degraded-evidence case an active high/critical finding elevates out of `insufficient_evidence`): `reason` is severity/findings-driven and does not name a gap, while `first_recommended_action` still names the selected gap when it is addressable.

  Every field projected into these lines is one-lined first (a gap subject is a tool name; a semantic gap's `path` embeds one), so a repository-derived value cannot forge a line. The action stays `kind: "info"` in all evidence cases — an evidence gap is closed by a reviewed declaration, never by a command Shipgate hands you.
