...
Runtime contract v25 adds one route to the control envelope:
...
and no proven policy weakening, when at least one open declaration question is
one the scan can answer from its own evidence. It is not published on a
ref-bound (`--base`/`--head`) run: `apply-patches` mutates the checkout, so the
exact rerun would re-scan the commit the edit is not in yet — the same
precondition every mechanical repair route carries. The PR flow is therefore
local loop, then commit, then CI verifies the committed result.
...
- **After it.** Re-run verification. If questions remain, they are the ones
  tagged `"human"`, and the human route names them rather than asking for
  generic review.
...
- Runtime contract: `25` (minimum control contract: `21`)
- Current report schema: `0.41` — [`docs/report-schema.v0.41.json`](report-schema.v0.41.json)
...
- Current verifier schema: `0.14` — [`docs/verifier-schema.v0.14.json`](verifier-schema.v0.14.json) (v0.13 and earlier stay frozen; `0.14` adds `authorable_by` to declaration questions and `fix_task.declaration_confirmation`)
...
schema [`docs/verifier-schema.v0.14.json`](verifier-schema.v0.14.json)). **Lead
...
- [`docs/report-schema.v0.41.json`](report-schema.v0.41.json) — machine-validatable JSON Schema for the current report.
