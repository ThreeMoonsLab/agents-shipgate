...
- **Evidence-gap actionability (v0.16+).** One **selected gap** feeds every short-form surface that names one: the first `release_decision.evidence_coverage.evidence_gaps[]` row whose `next_action.path` is a **nonblank normalized target** — the path with Unicode format characters (U+200B, U+202E, …) removed and whitespace/control runs collapsed; a raw string that normalizes to nothing names no surface and is not addressable. When no row is addressable it falls back to the first row. `Improve evidence:` (CLI and step summary, printed only on an `insufficient_evidence` verdict) always renders the selected gap's action. Two guarantees, scoped to exactly what holds:
  - **Alignment, on `insufficient_evidence` with an addressable gap.** When the verdict is `insufficient_evidence` *and* at least one gap carries a nonblank normalized `next_action.path`, `release_decision.reason`, `Improve evidence:`, and `first_recommended_action.why` name the **same** gap and the **same** path. The reason leads with that gap and reports the source-warning / low-confidence counts as `Context:`.
  - **No false dead end, on every verdict.** The phrase *"no machine-applicable fix is available"* is never emitted in `first_recommended_action.why` while any gap carries a nonblank normalized `next_action.path`. When you do see it, no gap names a surface to open and the next step really is a human gathering evidence.
  ...
  Every field projected into these lines is normalized first — Unicode format characters dropped, whitespace and control runs collapsed — so a repository-derived value (a gap subject is a tool name; a semantic gap's `path` embeds one) cannot forge a line or hide behind an invisible character. An affordance is published only when it survives that normalization: a `command` that normalizes to nothing produces no `Run:` line and a `null` repair command, and blank accepted values are dropped.

  On the **evidence-first** actions — `insufficient_evidence`, and `review_required` where evidence is below the threshold or drives the review outright — `first_recommended_action.kind` stays `"info"`. That is a statement about the *summary projection*, not about the gap rows: an evidence gap is usually closed by a reviewed declaration a human must write, but not always. A `provide_source` row whose `path` is `--diff-from` carries an exact regeneration command for a stale comparison base, `Improve evidence:` renders it after `Run:`, and `verify`'s human fix-task repair keeps it. Route on the row's `requires_human_review` / `auto_apply` / `suggested_patch_kind` fields rather than assuming either extreme. And it is not a promise about every `review_required` action: the auto-apply branch above returns `kind: "command"`.

  A gap counts as addressable when it names a nonblank normalized target **or** carries a publishable command; a command-only row is as actionable as one naming a file, and selection, the reason, and the no-dead-end rule all treat it that way. A command is published only if it is safe exactly as written — one containing control, bidi, or invisible code points is suppressed rather than rewritten, because deleting a zero-width character from a command changes which program runs.
