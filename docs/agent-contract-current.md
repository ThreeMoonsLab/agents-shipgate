...
`unread_fields_changed`, `changed_without_rows`, or on an incomparable result
`blocking_limit` with its `limit` kind) and `omitted_items`. It is evidence beside the rows; it moves no
