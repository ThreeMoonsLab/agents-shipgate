...
- **Evidence-verdict actionability (v0.16+).** On an evidence verdict, `release_decision.reason`, the short-form `Improve evidence:` line, and `first_recommended_action.why` all project the **same** selected gap: the first `release_decision.evidence_coverage.evidence_gaps[]` row whose `next_action.path` is non-null, falling back to the first row when none is addressable. Two consequences you can rely on: the reason leads with that gap and its path and reports the source-warning / low-confidence counts as `Context:`, and the phrase *"no machine-applicable fix is available"* is never emitted while any gap carries a non-null `next_action.path`. When you see it, no gap names a surface to open and the next step really is a human gathering evidence. The action stays `kind: "info"` either way — an evidence gap is closed by a reviewed declaration, never by a command Shipgate hands you.
