...
* it is never weaker than any reading or existing reviewed manifest constraint
  (see ``propose_effect_declaration``), so confirming one without thinking
  over-declares rather than under-declares — and it is offered only where
  something was actually observed, never from a protocol default standing in
...
def build_declaration_scaffold(
    ...
    *,
    ...
    lines = _header(
        questions,
        has_proposal_audit=any(entry["proposal_audit"] for entry in sections),
    )
    ...
    for gap in gaps:
        ...
        if existing is None:
            entry: dict[str, Any] = {
                ...
                "proposal_audit": _proposal_audit(action),
                "numbers": (),
            }
...
def _absorb(
    ...
    audit = _proposal_audit(action)
    if audit:
        entry["proposal_audit"] = audit
    ...
    *,
    ...
    proposal_guidance = (
        [
            ...
            "# printed above it and may also preserve existing reviewed manifest",
            "# constraints named in that block's proposal audit. The proposal is",
            "# never weaker than either. Keep it to confirm, or replace it with a",
            "# value you can defend. Nothing here is operative until you merge it",
            "# into shipgate.yaml yourself.",
        ]
    ...
    *,
    ...
    out.extend(
        _reading_lines(
            ...
            proposal_audit=entry["proposal_audit"],
        )
    )
    out.append("---")
    ...
    if "effect" in _proposed_fields(template):
        ...
        if proposal_audit:
            message = (
                ...
                "here and every existing reviewed manifest constraint named by "
                "this row, so confirming it can only over-declare. Replace it if "
                "you can defend a different reading. "
                f"Proposal audit: {proposal_audit}"
            )
        else:
            message = (
                f"Proposed below: {proposal}{detail} — at or above every reading "
        ...
        lines.extend(_wrapped_comment(message, ""))
    return lines
...
def _proposal_audit(action: Any) -> str:
    ...
    expects = str(getattr(action, "expects", "") or "")
...
def _drop_duplicate_blocks(sections: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for entry in sections:
        ...
        if not first["proposal_audit"]:
            first["proposal_audit"] = entry["proposal_audit"]
    return kept
...
def _annotate(
    ...
    if path in proposed:
        ...
        proposal_source = (
            "the evidence plus the existing reviewed manifest constraints named "
            "above"
            if proposal_audit
        ...
        out.extend(
            _wrapped_comment(
                f"proposed from {proposal_source} — keep it to confirm, or replace it.",
                pad,
            )
        )
    hint = _hint_for(path)
    if hint is not None:
        if proposal_audit and path.split(".")[-1] == "risk_tags":
            hint = (
                "filled in from what this scan observed above plus the existing "
                "reviewed manifest tags named in the proposal audit. Keeping "
                "these declares the categories as reviewed, which both accounts "
                "for the observation and makes each category's built-in controls "
                "apply to this action — the obligation the row is missing. "
                "Delete any you reject, and record that judgement in `override:` "
                "instead."
            )
