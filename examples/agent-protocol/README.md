...
These examples exercise the `shipgate check` / `agent_result_v1` control loop.

Run a fixture diff:

```bash
shipgate check --agent codex --workspace . --diff diffs/block-stop.diff --format agent-json
```
...
- `stale-install.json` — the instruction-level fallback when the binary is present but older than the required `>=0.13.0`.
