...
1. **Run a scan and locate the top finding.**
   ...
   - **`insufficient_evidence` verdict** (v0.14+; the scan saw too many low-confidence tools or 4+ source warnings, an unproven binding graph, or an unresolved semantic/policy gap). There is no specific finding to fix. Read `release_decision.evidence_coverage.evidence_gaps[]` and work the **selected** row — the first whose `next_action.path` is nonblank after normalization (v0.16+), which is also the row the action's `why` names and whose path it repeats. Then **branch on that row's `next_action.kind`**, because the rows are not all the same kind of work:
     - `provide_source` / `declare_tool_inventory` — a mechanical repair. It may carry an exact `command` (the stale-`--diff-from` base report is regenerated by a command, and its `path` is the `--diff-from` flag, not a file to open). Run what it gives you, then re-scan.
     - `declare_agent_root` / `declare_agent_bindings` / `provide_complete_binding_graph` / `resolve_binding_conflict`, and the `declare_action_effect` / `declare_action_authority` rows — reviewed **human declarations** about deployed wiring and authority. Open the `path`, surface the `accepted_values` to a human, and never auto-write them.
     - When no row carries a path there is nothing to open, and the `why` falls back to "gather deeper sources" (MCP/OpenAPI inputs, eval traces, additional source files) — that wording now means it truthfully.
