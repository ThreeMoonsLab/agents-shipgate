...
- **Verify run JSON**: `verify-run.json` uses `schema_version: "shipgate.verify_run/v5"`, embeds the content-addressed plan and executor, and binds unit-result and decision IDs. `run_id` is an exact compatibility alias of `request_id`; do not treat the run projection as a second gate.
- **Report JSON**: `report_schema_version: "1.0"` (frozen). Read `release_decision.decision` first. A `passed` decision requires a complete static binding graph from its entry points plus complete, conflict-free identity, effect, and authority evidence for every reachable action; it does not prove runtime behavior. Preserve `release_decision.static_analysis_only=true`, `runtime_behavior_verified=false`, and `static_verdict_disclaimer` in summaries. Read `release_decision.evidence_coverage.binding_coverage`, `semantic_coverage`, `identity_coverage`, and `policy_gap_count`, then work every `evidence_gaps[].next_action` in order. Binding, semantic, and policy-applicability gaps are not Findings and cannot be suppressed, baselined, severity-overridden, cleared by `--no-heuristics`, or satisfied by `human_ack`; binding and authority declarations are human assertions and must never be auto-written. An **effect** declaration may be written mechanically in exactly one case: the report tags that row `next_action.authorable_by: "coding_agent"`, and you apply it with the command `control.next_action` (kind `confirm_declarations`) names. Never fill in a row tagged `"human"`, never touch a `declaration_drift` row, and never weaken one the manifest already carries — propose what the evidence supports; only a person may assert against it. Use `tool_catalog[]` for diagnostics and `tool_inventory[]` for the proven reachable surface, and read `surface_exclusions` for what the run removed from analysis — a `newly_unbound_tool` row is a capability this change introduced that no check judged. The current schema is [`docs/report-schema.v1.0.json`](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/report-schema.v1.0.json); v0.43, the last pre-freeze version, is a frozen compatibility reference. `1.x` is additive-only, so a parser written against `1.0` keeps working across minors; a pre-freeze `0.x` report is no longer accepted as input to the CLI and must be regenerated. See the [current agent contract](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/agent-contract-current.md), [verification identity contract](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/verification-reproducibility.md), and [evidence-backed passed contract](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/passed-verdict-contract.md).
