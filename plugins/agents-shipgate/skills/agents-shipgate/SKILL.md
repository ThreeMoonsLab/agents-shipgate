...
3. Before running Shipgate CLI commands, require `agents-shipgate >=0.13.0`: run `command -v agents-shipgate` and `agents-shipgate --version`. If it is missing or older than 0.13.0, tell the user to run `pipx install agents-shipgate` and then `pipx upgrade agents-shipgate`; if `pipx` is unavailable, use `python -m pip install -U "agents-shipgate>=0.13"`. The Codex plugin supplies workflows, not the scanner binary.
