"""Pydantic models for the adoption-harness scorecard.

The harness writes one ``ScorecardV1`` JSON file per cell under
``.agents-private/adoption-sprint/<run-id>/<cell-id>/scorecard.json``.
Detector functions live in :mod:`harness.adoption.scorer.rules`; this module
only defines the data shape.

A JSON-Schema export of the same model lives at
``src/agents_shipgate/schemas/adoption_scorecard.py`` for contract
versioning alongside the rest of the agents-shipgate schemas.
"""
...
class ScorecardV1(BaseModel):
    ...
    Field semantics are spelled out in :mod:`harness.adoption.scorer.rules`.
    """
