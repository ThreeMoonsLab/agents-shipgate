...
_MCP_PROJECT_TOKEN_RE = re.compile(
    r"(?:^|[^a-z0-9])mcp(?:[^a-z0-9]|$)", re.IGNORECASE
)


def _mcp_source_tokens(text: str, *, typescript: bool = False) -> list[tuple[str, str]]:
    """Comment/string-aware tokens for zero-install idiom detection.

    This is intentionally the small structural subset the script needs: it
    identifies literal registration names but carries no metadata into a scan.
    The installed adapter performs the full extraction.
    """
    ...
    while index < len(text):
        ...
        if text.startswith("/*", index):
            ...
            continue
        if char == "/" and typescript and _zero_regex_can_start(tokens):
            end = _zero_regex_end(text, index)
            if end is not None:
                index = end
                continue
        if char in {"'", '"'}:
            ...
            while cursor < len(text):
                ...
                if current == "\\":
                    ...
                    if cursor >= len(text):
                        ...
                        break
                    escaped = text[cursor]
                    if escaped not in {quote, "\\", "/", "n", "r", "t", "b", "f"}:
                        valid = False
                        cursor += 1
                        break
                ...
                cursor += 1
            if valid and cursor <= len(text) and cursor > index + 1:
                tokens.append(("string", "".join(value)))
            index = max(cursor, index + 1)
            continue
        if char == "`":
            index = _zero_backtick_end(text, index, typescript=typescript)
            continue
        if char.isalpha() or char in {"_", "$"}:
            ...
            while cursor < len(text) and (
                ...
                cursor += 1
            tokens.append(("ident", text[index:cursor]))
            index = cursor
            continue
...
def _zero_regex_can_start(tokens: list[tuple[str, str]]) -> bool:
    if not tokens:
        return True
    kind, value = tokens[-1]
    if kind == "punct":
        return value in {"(", "{", "[", "=", ":", ",", ";", "!", "?"}
    return kind == "ident" and value in {
        "case",
        "delete",
        "return",
        "throw",
...
def _zero_regex_end(text: str, start: int) -> int | None:
    cursor = start + 1
    in_class = False
    while cursor < len(text):
        char = text[cursor]
...
def _zero_backtick_end(text: str, start: int, *, typescript: bool) -> int:
    if not typescript:
    ...
    while cursor < len(text):
        ...
        if text.startswith("${", cursor):
            cursor = _zero_template_expression_end(text, cursor + 2)
            continue
        cursor += 1
    return len(text)


def _zero_template_expression_end(text: str, start: int) -> int:
    depth = 1
    cursor = start
    while cursor < len(text):
        if text.startswith("//", cursor):
        ...
        if char in {"'", '"'}:
            ...
            continue
        if char == "`":
            cursor = _zero_backtick_end(text, cursor, typescript=True)
            continue
        if char == "{":
            depth += 1
        elif char == "}":
...
def _zero_matching(
    ...
    for index in range(start, len(tokens)):
        if tokens[index][1] == opening:
            depth += 1
        elif tokens[index][1] == closing:
            depth -= 1
            if depth == 0:
                return index
    return None
...
def _zero_mcp_constructor(
    tokens: list[tuple[str, str]], bounds: tuple[int, int]
) -> bool:
    start, end = bounds
    return any(
        tokens[index][1] == "mcp"
        and tokens[index + 1][1] == "."
        and tokens[index + 2][1] in {"NewTool", "Tool"}
        for index in range(start, max(start, end - 2))
    )


def _zero_sole_string(
    tokens: list[tuple[str, str]], bounds: tuple[int, int]
) -> bool:
    return _zero_sole_string_value(tokens, bounds) is not None


def _zero_sole_string_value(
    tokens: list[tuple[str, str]], bounds: tuple[int, int]
) -> str | None:
    start, end = bounds
    if end - start != 1 or tokens[start][0] != "string":
        return None
    return tokens[start][1]


def _zero_field_value_end(
    tokens: list[tuple[str, str]], start: int, end: int
) -> int:
    depth = 0
    for index in range(start, end):
...
def _zero_named_field_kind(
    tokens: list[tuple[str, str]], bounds: tuple[int, int], name: str
) -> str:
    value, present = _zero_named_field_value(tokens, bounds, name)
    if not present:
        return "absent"
    return "literal" if value is not None else "dynamic"


def _zero_named_field_value(
    tokens: list[tuple[str, str]], bounds: tuple[int, int], name: str
) -> tuple[str | None, bool]:
    start, end = bounds
    outer = next(
        (index for index in range(start, end) if tokens[index][1] == "{"),
        None,
    )
    if outer is None:
        return None, False
    ...
    for index in range(outer, end - 2):
        ...
        if (
            brace_depth == 1
            and tokens[index][1] == name
            and tokens[index + 1][1] == ":"
        ):
            value_end = _zero_field_value_end(tokens, index + 2, end)
            return _zero_sole_string_value(tokens, (index + 2, value_end)), True
    return None, False


def _zero_call_name_kind(
    tokens: list[tuple[str, str]], bounds: tuple[int, int], name: str
) -> str:
    value, present = _zero_call_name_value(tokens, bounds, name)
    if not present:
        return "absent"
    return "literal" if value is not None else "dynamic"


def _zero_call_name_value(
    tokens: list[tuple[str, str]], bounds: tuple[int, int], name: str
) -> tuple[str | None, bool]:
    start, end = bounds
    for index in range(start, end - 1):
        if tokens[index][1] != name or tokens[index + 1][1] != "(":
            continue
        close = _zero_matching(tokens, index + 1, "(", ")")
        if close is None or close > end:
            continue
        args = _zero_top_level_args(tokens, index + 2, close)
        if not args:
            return None, True
        return _zero_sole_string_value(tokens, args[0]), True
    return None, False


def _zero_tool_name_kind(
    tokens: list[tuple[str, str]], bounds: tuple[int, int]
) -> str:
    """Return literal, dynamic, or absent for a visible MCP tool value."""

    named = _zero_named_field_kind(tokens, bounds, "Name")
    return named if named != "absent" else _zero_call_name_kind(tokens, bounds, "NewTool")


def _zero_tool_name_value(
    tokens: list[tuple[str, str]], bounds: tuple[int, int]
) -> tuple[str | None, bool]:
    named = _zero_named_field_value(tokens, bounds, "Name")
    return named if named[1] else _zero_call_name_value(tokens, bounds, "NewTool")


def _zero_static_prefix(tokens: list[tuple[str, str]], index: int) -> bool:
    for cursor in range(index - 1, -1, -1):
        kind, value = tokens[cursor]
        if value in {";", "{", "}"}:
            return False
        if kind == "ident" and value == "static":
            return True
    return False


def _zero_class_bodies(
    tokens: list[tuple[str, str]],
) -> tuple[list[tuple[int, int]], int]:
    bodies: list[tuple[int, int]] = []
    unbalanced = 0
    for index, token in enumerate(tokens):
        if token != ("ident", "class"):
            continue
        brace = next(
            (cursor for cursor in range(index + 1, len(tokens)) if tokens[cursor][1] == "{"),
            None,
        )
        if brace is None:
            unbalanced += 1
            continue
        end = _zero_matching(tokens, brace, "{", "}")
        if end is None:
            unbalanced += 1
        else:
...
def _mcp_code_idiom_counts(path: Path) -> dict[str, tuple[list[str], int, int]]:
    """Return ``idiom -> (literal names, dynamic names)`` for one file."""

    try:
        if path.stat().st_size > MAX_MCP_CODE_FILE_BYTES:
            return {}
        text = path.read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError):
        return {}
    is_typescript = path.suffix.lower() in {
        ".ts",
        ".tsx",
        ".js",
        ".jsx",
    ...
    tokens = _mcp_source_tokens(text, typescript=is_typescript)
    facts: dict[str, list[Any]] = {idiom: [[], 0, 0] for idiom in MCP_CODE_IDIOMS}
    if is_typescript:
        bodies, unbalanced = _zero_class_bodies(tokens)
        facts["typescript_static_tool_v1"][2] += unbalanced
        for start, end in bodies:
            for index in range(start + 1, end):
                ...
                if (
                    kind != "ident"
                    or value != "toolName"
                    or not _zero_static_prefix(tokens, index)
                ):
                    continue
                cursor = index + 1
                while cursor < min(end, index + 12) and tokens[cursor][1] not in {"=", ";", "{"}:
                    cursor += 1
                if cursor < end and tokens[cursor][1] == "=":
                    expression_end = next(
                        (
                            expression_cursor
                            for expression_cursor in range(cursor + 1, end)
                            if tokens[expression_cursor][1] == ";"
                        ),
                        end,
                    )
                    literal = _zero_sole_string_value(
                        tokens, (cursor + 1, expression_end)
                    )
                    if literal is None:
                        facts["typescript_static_tool_v1"][1] += 1
                    else:
                        facts["typescript_static_tool_v1"][0].append(literal)
        for index, token in enumerate(tokens):
            kind, value = token
            if (
                kind == "ident"
                and value in {"tool", "registerTool"}
                and index >= 2
                and index + 1 < len(tokens)
                and tokens[index - 1][1] == "."
                and tokens[index + 1][1] == "("
            ):
                receiver = tokens[index - 2][1].replace("_", "").lower()
                if receiver == "mcp" or receiver.endswith("server"):
                    end = _zero_matching(tokens, index + 1, "(", ")")
                    if end is None:
                        facts["typescript_mcp_sdk_v1"][2] += 1
                        continue
                    args = _zero_top_level_args(tokens, index + 2, end)
                    literal = (
                        _zero_sole_string_value(tokens, args[0]) if args else None
                    )
                    if literal is None:
                        facts["typescript_mcp_sdk_v1"][1] += 1
                    else:
                        facts["typescript_mcp_sdk_v1"][0].append(literal)
    elif path.suffix.lower() == ".go":
        for index, token in enumerate(tokens):
            kind, value = token
            if kind == "ident" and value == "AddTool" and index + 1 < len(tokens):
                package_call = (
                    index >= 2
                    and tokens[index - 1][1] == "."
                    and tokens[index - 2][1] == "mcp"
                )
                if tokens[index + 1][1] != "(":
                    continue
                end = _zero_matching(tokens, index + 1, "(", ")")
                if end is None:
                    facts["go_addtool_v1"][2] += 1
                    continue
                args = _zero_top_level_args(tokens, index + 2, end)
                package_function = package_call and len(args) >= 3
                tool_arg_index = 1 if package_function else 0
                if len(args) <= tool_arg_index:
                    continue
                tool_arg = args[tool_arg_index]
                if (
                    not package_function
                    and not _zero_mcp_constructor(tokens, tool_arg)
                ):
                    continue
                name, present = _zero_tool_name_value(tokens, tool_arg)
                if name is not None:
                    facts["go_addtool_v1"][0].append(name)
                else:
                    # A package-level mcp.AddTool call is MCP-anchored even
                    # when the tool value is assembled elsewhere.
                    if package_function or present:
                        facts["go_addtool_v1"][1] += 1
    return {
        idiom: (list(fact[0]), int(fact[1]), int(fact[2]))
        for idiom, fact in facts.items()
        if fact[0] or fact[1] or fact[2]
    }


def _zero_go_musttool_facts(
    source_files: list[Path],
) -> list[tuple[Path, str | None, str | None]]:
    """Return one fact per static ``Identifier.Register(mcp)`` registration."""

    tokenized: list[tuple[Path, list[tuple[str, str]]]] = []
    for path in source_files:
        ...
        try:
            ...
            continue
        tokenized.append((path, _mcp_source_tokens(text)))

    definitions: dict[tuple[Path, str], list[tuple[Path, str | None]]] = {}
    for path, tokens in tokenized:
        for index, (kind, value) in enumerate(tokens[:-1]):
            if kind != "ident" or value != "MustTool" or tokens[index + 1][1] != "(":
                continue
            if (
                index < 5
                or tokens[index - 1][1] != "."
                or tokens[index - 2][0] != "ident"
                or not tokens[index - 2][1].replace("_", "").lower().startswith("mcp")
                or tokens[index - 3][1] != "="
                or tokens[index - 4][0] != "ident"
                or tokens[index - 5] != ("ident", "var")
            ):
                continue
            close = _zero_matching(tokens, index + 1, "(", ")")
            if close is None:
                continue
            args = _zero_top_level_args(tokens, index + 2, close)
            name = _zero_sole_string_value(tokens, args[0]) if args else None
            definitions.setdefault((path.parent, tokens[index - 4][1]), []).append(
                (path, name)
            )

    facts: list[tuple[Path, str | None, str | None]] = []
    for path, tokens in tokenized:
        for index, (kind, value) in enumerate(tokens[:-1]):
            if kind != "ident" or value != "Register" or tokens[index + 1][1] != "(":
                continue
            if index < 2 or tokens[index - 1][1] != ".":
                continue
            close = _zero_matching(tokens, index + 1, "(", ")")
            if close is None:
                continue
            ...
            if (
                len(args) != 1
                or args[0][1] - args[0][0] != 1
                or tokens[args[0][0]] != ("ident", "mcp")
            ):
                continue
            receiver = tokens[index - 2]
            if receiver[0] != "ident":
                facts.append((path, None, "dynamic_tool_registration"))
                continue
            matches = definitions.get((path.parent, receiver[1]), [])
            if not matches:
                facts.append((path, None, "unresolved_tool_registration"))
            elif len(matches) != 1:
                facts.append((path, None, "ambiguous_tool_registration"))
            elif matches[0][1] is None:
                facts.append((matches[0][0], None, "dynamic_tool_name"))
            else:
                facts.append((matches[0][0], matches[0][1], None))
    return facts


def _zero_mcp_project_marker(project: Path, matched: list[Path]) -> bool:
    for path in [project / "package.json", project / "go.mod", *matched[:32]]:
        if not path.is_file():
            continue
        try:
            ...
            continue
        if path.name in {"package.json", "go.mod"}:
            if "modelcontextprotocol" in text or "@mongodb-js/mcp" in text:
                return True
            if _MCP_PROJECT_TOKEN_RE.search(text):
                return True
            if any(
                token in text
                for token in (
                ...
                return True
        elif _zero_mcp_source_import_marker(path, text):
            return True
    return False


def _zero_mcp_source_import_marker(path: Path, text: str) -> bool:
    is_go = path.suffix.lower() == ".go"
    tokens = _mcp_source_tokens(text, typescript=not is_go)
    for index, (kind, value) in enumerate(tokens):
        if (
            ...
            and tokens[index + 1][1].replace("_", "").lower().startswith("mcp")
        ):
            return True
        if kind != "ident" or value not in {"import", "require"}:
            continue
        end = min(len(tokens), index + 64)
        if index + 1 < len(tokens) and tokens[index + 1][1] == "(":
            matched = _zero_matching(tokens, index + 1, "(", ")")
        ...
        if any(
            ...
            and (
                "modelcontextprotocol" in candidate_value.lower()
                or _MCP_PROJECT_TOKEN_RE.search(candidate_value) is not None
            )
            for candidate_kind, candidate_value in tokens[index + 1 : end]
        ):
            return True
    return False


def _zero_mcp_code_suggestions(
    workspace: Path, files: list[Path], preferred_exports: list[str]
) -> tuple[list[dict[str, str]], dict[str, Any] | None, str | None]:
    candidates = [
        path
        for path in files
        if path.suffix.lower() in {".ts", ".tsx", ".js", ".jsx", ".mts", ".cts", ".go"}
        and not any(part.lower() in {"test", "tests", "testdata", "vendor", "fixtures", "node_modules"} for part in path.parts[:-1])
        and not path.name.lower().endswith((".test.ts", ".test.tsx", ".spec.ts", ".spec.tsx", "_test.go"))
    ]
    source_files: list[Path] = []
    scheduled_bytes = 0
    cap_reason: str | None = None
    for path in candidates:
        if len(source_files) >= MAX_MCP_CODE_FILES:
            cap_reason = (
                "MCP code idiom discovery stopped at its aggregate "
                f"{MAX_MCP_CODE_FILES}-file limit; later source files were not surveyed"
            )
            break
        try:
            ...
            cap_reason = (
                f"MCP code idiom discovery could not inspect {path.name}; the "
                "source survey is incomplete"
            )
            break
        if size > MAX_MCP_CODE_FILE_BYTES:
            cap_reason = (
                f"MCP code idiom discovery did not read {path.name} because it "
                f"exceeds the {MAX_MCP_CODE_FILE_BYTES}-byte per-file limit; the "
                "source survey is incomplete"
            )
            break
        if scheduled_bytes + (2 * size) > MAX_MCP_CODE_TOTAL_BYTES:
            cap_reason = (
                "MCP code idiom discovery stopped at its aggregate "
                f"{MAX_MCP_CODE_TOTAL_BYTES}-byte read limit; later source files "
                "were not surveyed"
            )
            break
        scheduled_bytes += 2 * size
        source_files.append(path)
    grouped: dict[tuple[str, Path], list[Path]] = {}
    literal_counts: dict[tuple[str, Path], int] = {}
    literal_names: dict[tuple[str, Path], set[str]] = {}
    dynamic_counts: dict[tuple[str, Path], int] = {}
    structural_counts: dict[tuple[str, Path], int] = {}
    link_omission_counts: dict[tuple[str, Path], int] = {}

    def project_of(path: Path) -> Path:
        found = _project_of(path, workspace)
        return workspace if found is None or found[0] == "." else workspace / found[0]

    for path in source_files:
        for idiom, (names, dynamic_count, structural_count) in _mcp_code_idiom_counts(
            path
        ).items():
            project = project_of(path)
            key = (idiom, project)
            grouped.setdefault(key, []).append(path)
            literal_counts[key] = literal_counts.get(key, 0) + len(names)
            literal_names.setdefault(key, set()).update(names)
            dynamic_counts[key] = dynamic_counts.get(key, 0) + dynamic_count
            structural_counts[key] = (
                structural_counts.get(key, 0) + structural_count
            )
    for path, name, reason in _zero_go_musttool_facts(source_files):
        key = ("go_musttool_v1", project_of(path))
        grouped.setdefault(key, []).append(path)
        if name is not None:
            literal_counts[key] = literal_counts.get(key, 0) + 1
            literal_names.setdefault(key, set()).add(name)
        elif reason == "dynamic_tool_name":
            dynamic_counts[key] = dynamic_counts.get(key, 0) + 1
        else:
            link_omission_counts[key] = link_omission_counts.get(key, 0) + 1
    grouped = {
        key: matched
        for key, matched in grouped.items()
        if _zero_mcp_project_marker(key[1], matched)
    }
    literal_counts = {
        key: value for key, value in literal_counts.items() if key in grouped
    }
    dynamic_counts = {
        key: value for key, value in dynamic_counts.items() if key in grouped
    }
    link_omission_counts = {
        key: value for key, value in link_omission_counts.items() if key in grouped
    }
    structural_counts = {
        key: value for key, value in structural_counts.items() if key in grouped
    }
    preferred_names: dict[Path, set[str]] = {}
    for relative in preferred_exports:
        names = _zero_mcp_export_tool_names(workspace / relative)
        if names:
            preferred_names.setdefault(project_of(workspace / relative), set()).update(
                names
            )
    suggestions: list[dict[str, str]] = []
    evidence: list[str] = []
    candidates: list[str] = []
    for (idiom, project), matched in sorted(
        grouped.items(), key=lambda item: (item[0][1].as_posix(), item[0][0])
    ):
        unique = sorted(set(matched))
        source = unique[0] if len(unique) == 1 else Path(os.path.commonpath([str(path) for path in unique]))
        relative = _rel(source, workspace)
        candidates.append(relative)
        literal_count = literal_counts.get((idiom, project), 0)
        dynamic_count = dynamic_counts.get((idiom, project), 0)
        measured = f"{literal_count} literal tool registration(s)"
        if dynamic_count:
            measured += f" and {dynamic_count} runtime-built name(s) omitted"
        link_count = link_omission_counts.get((idiom, project), 0)
        if link_count:
            measured += f" and {link_count} unresolved static registration link(s) omitted"
        structural_count = structural_counts.get((idiom, project), 0)
        if structural_count:
            measured += f" and {structural_count} static recognition gap(s) omitted"
        names = literal_names.get((idiom, project), set())
        export_covers_code = (
            bool(names)
            and not dynamic_count
            and not link_count
            and not structural_count
            and names <= preferred_names.get(project, set())
        )
        if export_covers_code:
            evidence.append(
                f"built-in MCP code idiom {idiom}: {measured} under {relative}; "
                "a checked-in "
                "MCP export in the same project is the preferred source"
            )
        else:
            suggestions.append({"type": "mcp", "path": relative, "idiom": idiom})
            evidence.append(
                f"built-in MCP code idiom {idiom}: {measured} under {relative}"
            )
    if not grouped:
    ...
    evidence_count = (
        sum(literal_counts.values())
        + sum(dynamic_counts.values())
        + sum(link_omission_counts.values())
        + sum(structural_counts.values())
    )
    score = 2.0 + min(2.0, 0.25 * evidence_count)
    detection = {
        "type": "mcp_server",
        "score": round(score, 2),
    ...
    if cap_reason is not None:
        detection["evidence"].append(cap_reason)
    return suggestions, detection, cap_reason


def _zero_mcp_export_tool_names(path: Path) -> set[str]:
    """Read static names from an aggregate JSON export or snapshot directory."""

    candidates = (
        sorted(path.rglob("*.snap"), key=lambda candidate: candidate.as_posix())
        if path.is_dir()
        else [path]
    )
    names: set[str] = set()
    scheduled = 0
    for candidate in candidates[:MAX_MCP_CODE_FILES]:
        try:
            size = candidate.stat().st_size
        except OSError:
            continue
        if size > MAX_MCP_CODE_FILE_BYTES or scheduled + size > MAX_MCP_CODE_TOTAL_BYTES:
            break
        scheduled += size
        try:
        ...
        if isinstance(raw_tools, list):
            names.update(
                ...
                and isinstance((raw_name := row.get("name")), str)
                and raw_name
            )
    return names


def _zero_mcp_snapshot_suggestions(
    workspace: Path, files: list[Path]
) -> tuple[list[dict[str, str]], dict[str, Any] | None, list[dict[str, str]]]:
    """Aggregate GitHub MCP's checked-in ``__toolsnaps__/*.snap`` exports."""

    roots = sorted(
        {
            path.parent
            for path in files
            if path.suffix.lower() == ".snap" and path.parent.name == "__toolsnaps__"
        },
    ...
    for root in roots:
        relative = _rel(root, workspace)
        candidates = sorted(
            (path for path in files if path.parent == root and path.suffix.lower() == ".snap"),
            key=lambda path: path.as_posix(),
        )
        omissions = max(0, len(candidates) - MAX_MCP_CODE_FILES)
        names: list[str] = []
        scheduled = 0
        for candidate in candidates[:MAX_MCP_CODE_FILES]:
            try:
            ...
            if size > MAX_MCP_CODE_FILE_BYTES:
                omissions += 1
                continue
            if scheduled + size > MAX_MCP_CODE_TOTAL_BYTES:
                omissions += len(candidates) - len(names)
                break
            scheduled += size
            try:
                payload = json.loads(candidate.read_text(encoding="utf-8"))
            ...
            excluded.append(
                {
                    "type": "mcp",
                    "path": relative,
                    "reason": "MCP tool snapshot directory contained no loadable tools",
                }
            )
            continue
        suggestions.append(
            {"type": "mcp", "path": relative, "idiom": MCP_TOOL_SNAPSHOT_V1}
        )
        candidate_files.append(relative)
        measured = f"{len(names)} checked-in per-tool MCP snapshot(s)"
        if omissions:
            measured += f" and {omissions} omission(s)"
        evidence.append(f"high-confidence MCP snapshot export: {measured} under {relative}")
    if not suggestions:
...
def _zero_merge_mcp_detections(
    snapshot: dict[str, Any] | None, code: dict[str, Any] | None
) -> dict[str, Any] | None:
    if snapshot is None:
        return code
    if code is None:
        return snapshot
    score = max(float(snapshot["score"]), float(code["score"]))
    return {
        "type": "mcp_server",
        "score": score,
        "confidence": _confidence(score),
        "evidence": [*snapshot["evidence"], *code["evidence"]],
        "candidate_files": sorted(
            set([*snapshot["candidate_files"], *code["candidate_files"]])
        ),
    }
...
def detect(workspace: Path) -> dict[str, Any]:
    ...
    snapshot_sources, snapshot_detection, snapshot_excluded = (
        _zero_mcp_snapshot_suggestions(workspace, files)
    )
    suggested.extend(snapshot_sources)
    excluded.extend(snapshot_excluded)
    mcp_code_sources, mcp_code_detection, mcp_code_cap_reason = _zero_mcp_code_suggestions(
        workspace,
        files,
        [source["path"] for source in suggested if source["type"] == "mcp"],
    )
    suggested.extend(mcp_code_sources)
    if mcp_code_cap_reason is not None:
        excluded.append(
            {
                "type": "mcp",
                "path": ".",
                "reason": mcp_code_cap_reason,
                "reason_code": "walk_capped",
            }
        )
    mcp_detection = _zero_merge_mcp_detections(
        snapshot_detection, mcp_code_detection
    )
    if mcp_detection is not None:
    ...
    elif mcp_code_cap_reason is not None:
        next_action = (
            f"{mcp_code_cap_reason}. Re-run detect with --workspace pointed at "
            "the MCP server project rather than the enclosing monorepo."
        )
