...
def _decode_typescript(body: str) -> str | None:
    ...
    while index < length:
        ...
        if marker == "u":
            ...
            continue
        if marker.isdigit():
            # Legacy octal (`\1`-`\7`) is a syntax error under `use strict` and
            # in a template literal, and octal *elsewhere*; `\8`/`\9` are their
            # own special case. Which one a file means depends on a mode this
            # reader does not track, so it refuses rather than pick.
            return None
        out.append(marker)
        index += 2
    return "".join(out)
...
def _opens_regex(out: list[str], text: str, index: int) -> bool:
    ...
    if previous == "" or previous in _REGEX_PRECEDING_CHARS:
        return True
    if previous == ")":
        # A `)` is usually the end of a call, and `foo(a) / 2` divides. But it
        # is also the end of a control statement's condition, and there a regex
        # validly begins the body: reading the `/` in
        # `if (ok) /\.registerTool("fake", h)/.test(v)` as division scanned the
        # pattern as code and reported a `fake` tool. Which of the two it is,
        # is decided by the keyword in front of the matching `(`.
...
def _consume_quoted(masker: _Masker, start: int, quote: str, *, allow_newline: bool) -> int:
    text = masker.text
    length = len(text)
    index = start + 1
    while index < length:
        ...
        if char == "\n" and not allow_newline:
            break
        index += 1
    # Unterminated. Blank to the resync point but record no literal, and say
    # so: past here this reader cannot tell code from content.
    end = text.find("\n", start)
...
def _literal_is_whole_value(source: MaskedSource, end: int, terminators: str) -> bool:
    ...
    can see, and reading it would publish ``backup`` for a tool the server
    registers under some other name.
    """
    index = end
