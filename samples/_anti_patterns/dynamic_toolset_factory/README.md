...
- `agents-shipgate detect --json --workspace .` returns `is_agent_project: false` and `frameworks: []`. The fixture defines a local `class Agent` rather than importing `from agents import Agent`, so the AST walker sees no SDK import to detect. `agent_name_candidates` carries no `Agent_name_literal` entry either: name extraction resolves each callee through its binding, and a spelling bound to a local `class` in the same file is not a framework constructor — reading it as one is how a fabricated identity reaches a manifest. This is a deliberate detail of the fixture: anti-pattern samples shouldn't pretend to be real agent projects — they isolate one failure mode at a time, and the failure mode here is *inventory enumeration*, not *project detection*.
