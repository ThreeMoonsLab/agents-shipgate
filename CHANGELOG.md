...
- `diff --application` reads the tool list an agent-building function receives from its caller. On mezgoodle/Vesta#58, `create_secretary_agent(tools, model)` builds `LlmAgent(name="SecretaryAgent", tools=tools)` and the service calls it with `tool_groups["calendar"] + tool_groups["email"]`, where `tool_groups` is the dict `create_tools(...)` returns. Every agent read "uses a dynamic tools expression" and the PR produced no row. It now shows `SecretaryAgent` gaining `check_emails` (the user's Gmail inbox) with the calendar tools, and replacing `CalendarAgent` under the root agent; each row names the call site (`supplied by the call at app/services/adk_service.py:123`) and the definition. (#874)
  - **What is followed.** In both readers, a non-literal `tools=`/`handoffs=`/`sub_agents=` is evaluated from source: names bound once to literal lists that nothing changes, `+`/`*` splices, `x or [...]` by truthiness, literal-key subscripts and `.get` of a dict literal a factory returns, a factory's single `return`, and a module-level function's parameter at its one call site in the read scope (to a depth of four functions). Each element is resolved in its own module.
  - **What stays a named limit.** A census of the read scope finds the builder's calls. Two call sites, none, a reference other than a direct call (passed as a value, named in a string), a decorator, a link, `*`/`**` arguments, or a parameter the builder changes stop the evaluation; the agent keeps its dynamic-tools limit with the reason appended. Lists from two call sites are never merged. Test files are not call sites. Instance attributes (`self.tools`) and a `name=` passed as a parameter are not followed yet.
  - **Evidence.** Rows reached this way add `parameter_flow`: the call site and every module read, with digests, outside the compared meaning. Additive on the unreleased `0.1` advisory result; no schema or contract change.
