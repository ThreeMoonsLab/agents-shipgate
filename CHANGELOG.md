...
- A Claude Code hook row now states whether anything selects the hook file:
  ...
  - **A hook a plugin selects** is read as `execute`/`medium`, but is not an expansion, because plugin enablement is not in the repository. That covers `hooks/hooks.json` at a root a `.claude-plugin/plugin.json` or a `./`-sourced `.claude-plugin/marketplace.json` entry names, a `./` path in `hooks`, and inline hooks.
  ...
  - **Limits:** an unreadable manifest reference is a blocking limit for `audit --host`, `diff` and `verify`, while marketplace, skipped-directory and missing-file references are named without blocking. `check` leaves plugin-reference limits out, as in `1.0.0`.
  - **Baselines:** a host-grants baseline that recorded such a file as `execute` reports one changed, non-widening row. No schema or contract version moves. (#714)
