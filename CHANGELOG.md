...
- `agents-shipgate agent control --workspace <repo>` without `--reports-dir` now reads `<repo>/agents-shipgate-reports`, where `verify --workspace <repo>` publishes by default (one shared rule), instead of `agents-shipgate-reports` under the caller's current directory. From any other directory a valid run read as `missing`, and a caller standing in another verified repository had that repository's pointer read and refused against the requested one. Refusals now name the absolute directory searched, and the printed `verify` recovery writes back to it with an absolute `--out`. An explicit `--reports-dir` keeps its meaning (absolute as given, relative against the current directory), and nothing else is searched when it is absent. A reports directory outside the repository, such as a sibling `verify --out`, is no longer handed to Git as a change-set exclusion, so that generation refreshes instead of refusing as `workspace_unverifiable`. Compatibility: only an omitted `--reports-dir` combined with a `--workspace` other than the current directory reads a different directory than before; `--workspace .` output is unchanged. (#575)
