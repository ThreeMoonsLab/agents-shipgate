...
- A host comparison names the changed inputs it does not read, so a zero-row result is not read as covering them. (#821; slice 2 of #812)
  ...
  - **What it is not.** Never a row, a widening, a `check` violation or a claim that a host loads the file. Nothing is fetched or run, only plugin manifests and marketplaces are read, and at most 32 candidates are examined, the rest counted. The rules are listed in `docs/host-boundary-support.md` under *Changed inputs named but not read*.
  ...
  - **One route moves.** A manifest-free `verify` whose only host-relevant change is such an input now publishes the host comparison (advisory, exit `0`) instead of the setup route, which said nothing about the change. Rows, digests, baselines, `audit --host`, `check` and the benchmark replays are unchanged.
