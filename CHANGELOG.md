...
- Follow a tool an agent binds from another module in the repository. (#864)
  ...
  - **Identity.** One agent binding two different functions under one name binds neither, in both readers and whatever their order, and names both definitions. `import a.b` then `a.b.f` reads the submodule, as the import system does. For `scan`, a definition that an import reaches and another configured source also reads is one catalog tool, and the binding reaches it through the exact definition the reader resolved; a `{tool: …}` selector for it is not ambiguous. The module-binding walk is linear in the tree.
