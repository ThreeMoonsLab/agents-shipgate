...
- `check` no longer exits 1 with a traceback when a changed `.claude/settings.json`, `.claude/settings.local.json` or `.cursor/cli.json` sets a top-level key Shipgate does not model: `enabledPlugins` (added, set `false`, or turned from `true` to `false`), `extraKnownMarketplaces`, `outputStyle`, `enableAllProjectMcpServers`, `enabledMcpjsonServers` or any other key outside the host-boundary allow-list, alone or beside another change. In `1.0.0` every maintained format (text, `agent-boundary-json`, `agent-control-json`) raised a `ValidationError` ("publication authority requires complete boundary input coverage") for a worktree or `--base`/`--head` check, for any `--agent`, and printed no `next_action` in agent mode. The key's `SHIP-HOST-BOUNDARY-CONFIG-PARSE-FAILED` row (evidence kind `unknown_host_config_key`) counted as read input for publication but as unread input for coverage; both now take one shared answer. Such a check now answers `require_review`: a key-only change is `agent_action_required`, routed to `verify` with the row in `pending_review`, and a change that also adds a high-risk rule keeps its human route; none may merge or report completion. `input_coverage: complete` on these results means the file was read, not that every key's meaning is modelled — the row still owes review, and its title and recommendation are unchanged. Content that could not be read, such as malformed JSON, stays `partial` and unpublished. An input issue with no row of its own, such as an unreadable legacy `policies/host-boundary.shipgate.yaml`, is no longer treated as explained by an unknown-key row: `check` adds `BOUNDARY-INPUT-INCOMPLETE` and stops for a human, and `verify` reports `SHIP-AGENT-BOUNDARY-INPUT-INCOMPLETE`, as both already did beside any other change. Two answers change on input that did not crash: a mixed change such as `Bash(*)` plus `enabledPlugins` is still `block`/`human_review_required` but reports `complete` coverage instead of `partial`, and `check --diff` and the MCP `shipgate.check` tool on such input are `comparable` with host rows instead of `incomparable` (`diff_input_coverage_incomplete`) with none. Audit ids, the deprecated `codex-boundary-json` output, schemas and check ids are unchanged. (#810)
