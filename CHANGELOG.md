...
- The host diff reads the direction of three ordinary Claude Code permission edits correctly. Each one put a ⚠ widening on a change that widened nothing, and two changed `check`'s decision. `diff` text and `--json`, `check` rows and decisions, `verify`'s `host_comparison` and `audit --host --drift` expansion signals agree on each. No command, schema, check id or JSON member is added. (#816)
  ...
  - **Moved rules:** a rule whose identical text moved between `deny`, `ask` and `allow` in the same settings file is set aside before a one-out, one-in replacement is paired. `Bash(git status *)` → `Bash(git status --short *)` in the same edit that moves `Bash(git log *)` from `deny` to `allow` was "4 change(s), 3 widening"; it is now "4 change(s), 2 widening": the moved rule's allow row and deny removal. `check` was already right and is unchanged. Rules are matched by exact text only.
