...
- Add `examples/github-actions/14-host-only-advisory-pr.yml`, an advisory PR recipe for a repository with coding-agent host configuration and no `shipgate.yaml` or saved baseline: the pinned Action compares each PR with its base branch, keeps one comment up to date, falls back to the job summary without comment permission, and never fails the job. A test runs the recipe's inputs through the Action's own run step on manifest-free repositories; a GitHub-hosted run on a real pull request is still outstanding. (#780)
