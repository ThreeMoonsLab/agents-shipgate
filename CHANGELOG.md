...
- Stop publishing credential-shaped workflow labels verbatim. A job id GitHub accepts may look like a token (`ghp_…`, `AKIA…`), and a step name may carry registry credentials (`Pull docker://ci:<password>@gcr.io/proj/img`); both reached the host inventory, saved baselines, drift JSON, `diff`, `check` (rows and `evidence.job`/`evidence.scope`), manifest-free `verify`, `verifier.json` and the PR comment. Published `1.0.0` already did this for job ids in permission contexts, reusable calls and write-scope prefixes, and the unreleased step and secret-mapping rows added more places. Each job id, step `id`/`name`, trigger and permission scope name is now published once, where the workflow grant is built, through the report redactor, the host sanitizer and a `scheme://<redacted>@` userinfo rule, and every field, row and evidence entry uses that label; `config_sha256` is computed over it. Ordinary names such as `build`, `deploy-prod`, `secret-scan` and `token-refresh` are unchanged. A lone redacted label still compares, so a workflow with one token-shaped job id keeps complete coverage and every other row in the pull request; two distinct job ids, triggers or scope names in one workflow that publish alike are a blocking limit instead of comparing as one. Host-grants `0.6` and contract 40 are extended in place with no member added. (#802)
