...
- `verify` refuses an output directory that holds repository content, and every reader refuses a pointer published into one. A run leaves its output directory out of every working-tree read (change set, bound overlay, manifest-free host comparison, static input census), and `agent control` leaves the reports directory out of the change set it checks, so that a run's own reports never count as part of the change. An in-repository `--out` holding anything else hid that content. On published `1.0.0`, `verify --base main --out .claude` over a widened `.claude/settings.json` answered `complete`, `permissions.merge: true` and `merge_verdict: mergeable` with no findings, where the default directory answers `human_review_required` with `SHIP-HOST-BOUNDARY-PERMISSION-ALLOW-EXPANDED`. That held for a committed widening, an uncommitted one, and a new untracked settings file. On `main` it also held for a staged or committed deletion of a settings file carrying deny rules (`SHIP-HOST-BOUNDARY-PERMISSION-DENY-REMOVED`), which leaves no directory behind. `--out docs`, or `--out tools` for a manifest-named tool source, kept a `complete` pointer current across every later edit there, and a symlink into the repository or a case-variant spelling reached the same directories.
  - **The writer:** `verify`, `--head`, `--preview` and manifest-free `verify` now exit `2` with `config_error`, before writing anything, when the output directory is inside the repository and holds repository content. That means a path committed there at `HEAD`, a path the evaluated head or merge base held there that the change removes, or a staged or unignored untracked path that is not a Shipgate artifact. Staged generated reports stay a warning. The message names the directory and what it holds, and the next action is to choose another directory.
  - **The readers:** `agent control`, `verify --format control` and the human-review decision reader apply the same rule on every read, against `HEAD` and the merge base the pointer records. They refuse a pointer in such a directory as `workspace_unverifiable` in every control state, including a directory that gained content after the run, and a classification that cannot run refuses too. `agent control`'s recovery for this refusal is `verify` without `--out`, rather than the producing run's command, which would be refused.
  - **Identity:** the directory is judged by physical identity, so a symlinked or case-variant spelling is classified as the directory it reaches. Change-set exclusions now spell each existing component as it is stored, so a case-variant `--out` beneath the repository excludes the directory the run writes into; before, Git matched nothing and the run read its own reports as changes.
  - **Unchanged:** a gitignored directory, the default `agents-shipgate-reports`, a directory holding only Shipgate artifacts, a sibling directory outside the repository, and `verify --preview` outside Git. The Action's `--head` decisions were never affected, and `diff` and `check` take no output directory. No schema, contract, error kind, refusal code or exit code moves.
  - **Compatibility:** pointers `1.0.0` published into such a directory stop reading as current after upgrade; re-run `verify` into another directory. The Action's `output_dir` follows the same rule, and a default `agents-shipgate-reports` holding committed files or unrelated unignored files is refused like any other directory. See the [STABILITY.md migration note](STABILITY.md#output-directory-repository-content-804). (#804)
