...
- Every host comparison now says what it established, source by source, so a reviewer can tell "compared, no supported change" from "this input was not established" without running `audit --host`. Before, `diff`, `verify` and the manifest-free PR comment printed the same "No static host-grant changes detected" for a docs-only change, for an `env` or `apiKeyHelper` edit to `.claude/settings.json` that this entry does not read, and for a deleted settings file that held only such fields, while `verifier.json`'s own inventory digests differed; an incomparable result printed `head_inventory_incomplete` and named no file. (#812, slice 1; relevant unread candidates are #821, per-scope rows #808)
  - **The block:** `What this run established:` follows the entries, the no-change line or the cannot-compare lines. `.claude/settings.json (claude-code): compared; 3 rows`; `compared; observed a change in fields this entry does not read, so no row`; `read in base only; 2 rows` for a deleted file and `read in head only; 1 row` for a new or untracked one such as `.claude/settings.local.json`; `docs/proxy (claude-code, codex, cursor): unreadable in base and head, so neither inventory is complete` for a refused comparison; and one line, `compared with no change in what this entry reads: <first three> and N more`, for the quiet sources.
  - **JSON:** verifier schema `0.20` adds `host_comparison.coverage` and `shipgate diff --json` moves to capability diff `0.3` with the same `coverage` object: at most ten items of `{source, hosts, side, status, rows, limit, detail, scope}`, ordered so the cap drops a quiet source first, and `omitted_items`. `status` is `compared`, `unread_fields_changed` or, on an incomparable result, `blocking_limit` with the issue kind in `limit`. Sources are the inventory's redacted published paths; a source named in `unchanged_limits` is not repeated; `scope` is reserved for #808 and always `null`. Runtime contract v40, unreleased, is extended in place.
