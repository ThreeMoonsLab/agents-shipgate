...
- `diff --application` no longer answers `compared` with no rows when an agent it never observed gains a tool, and no longer refuses a repository over a tool name a test defines twice. (#876; the #873 rule applied the other way round: an unobserved agent is not "no change")
  - **The problem.** On kkmiecik-coder/CRM#5, `NARZEDZIA_WYCENY` gained `wyslij_obraz`, a tool that sends an image to the customer, bound by `zbuduj_agenta_wyceny()`, which does `return Agent(name="Wycena", …, tools=NARZEDZIA_WYCENY)`. The SDK reader read only `name = Agent(...)`, so none of the four builders in `agenci.py` was observed or named, and the only agent established was a test double in `tests/test_pro_tura.py`. The result was `compared`, `No established binding/interface/implementation changes`. Separately, a tool name defined twice in one test file exited 2 on whole-repository comparisons (alliance-genome/agr_ai_curation#842, usestrix/strix#1103, VRSEN/agency-swarm#796 and #804), telling the user to remove the duplicate from their test.
  - **Returned agents.** The OpenAI Agents SDK reader now also establishes `return Agent(name="Quote", ...)` in a function or method, keyed by its literal `name=` (the identity the ADK reader and the #873 "still named" rule already use), with the same location, binding and implementation evidence as an assigned agent. A builder's `tools=` parameter is its caller's to supply, so it is a dynamic tools expression, never a module-level list that happens to share its name; the reader now resolves a list name in the scope Python would. `Agent(name="x", **config)` or `Agent(*args)` without `tools=` is named as unpacking arguments that may set its tools, where it was read as a complete, empty tool list.
  - **Every other construction is named.** An SDK `Agent(...)` the reader recognises but does not establish — passed inline (`handoffs=[Agent(...)]`, a list or dict of agents, a call argument), assigned to an attribute or to several names, in a `lambda` or `yield`, returned without a literal `name=`, a parameterized `Agent[Context](...)`, a `.clone()` of an agent the module assigns, or a class deriving from `Agent` — is a coverage gap over its file that names its line (`OpenAI Agents SDK agent construction at agents.py:12 is not read (…); its agent and tool bindings are not established.`). The ADK reader already reads every agent call; a class deriving from an ADK agent class is named the same way. `compared` therefore holds no unaccounted construction site; `tests/test_application_diff_constructions.py` checks that against an independent census of 18 construction forms. One identity constructed at two sites in a file (two builders returning `Agent(name="Assistant", …)`, or two ADK agents sharing a `name=`) still binds the union of their tools, and is now also a gap naming both lines, so a tool moved between the two is never read as no change.
  - **Test code does not establish the application.** A file in a `test/` or `tests/` directory, or named `test_*.py`, `*_test.py`, `conftest.py`, `test.py` or `tests.py` (discovery's existing rule), relative to the selected scope, is not read, and nothing in it is a gap: each side names it in `limits` (`Test code is not read as the application (1 file(s)): tests/test_turn.py`) without making the result `partial`. A scope selected inside a test directory is read in full, on both sides, and a `*.py` link onto test code is a gap.
  - **A tool defined twice is a limit.** A tool name one application file defines twice is a coverage gap on that name in that file; no agent binds either definition, and every other file and tool is still compared. It never refuses the comparison.
  - **`scan`.** A declared SDK source reads returned agents, list names and unpacked arguments the same way, so its binding graph now includes builder-returned agents. The unread-construction census is published only to the application comparison for now.
  - **Re-measured.** On 2026-09-25, against the pinned corpora — the 134 open SDK/ADK pull requests at their pinned scopes and 48 of the 50 that modify an existing agent, at the repository root (two have no cached clone) — main `12b50e7c` answered `compared` on 3 and exited 2 on 3; this change answers `compared` on none, so no case is `compared` with a construction site unaccounted, and refuses none. Two were CRM-shaped silent cases: kkmiecik-coder/CRM#5 now establishes all four builders and names `NARZEDZIA_WYCENY` as an unresolved tool on `Wycena` (#864 resolves it), and cassao29/strix#1, whose only established agents were three test doubles, is `not_established`. tensorflow#128063 keeps its three `ADDED` rows and is `partial`, because its root agent and its builder share `name=`. agr_ai_curation#842 and agency-swarm#796/#804 compare instead of exiting 2, as does usestrix/strix#1103 at the repository root. Rows read from test code are gone from ITMO-NSS-team/FEDOT.MAS#38 and liauto-siada/siada-cli#6. The most common unread form is `Agent[Context](...)` (57 sites in 6 pull requests), then `return Agent(name=<module constant>)` (36 sites in 10). No application comparison schema change; `application_comparison_schema_version` stays `"0.1"`.
