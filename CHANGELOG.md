...
- Add the GTM plan of record (`marketing/gtm-strategy.md`), launch kit,
  design-partner outreach kit, and launch blog draft; README leads with the
  verifier PR-comment verdict ("What your PR sees") and links the
  coding-agent install path from the quickstart. First 12 free
  `cursor-static` adoption-benchmark rows land in `benchmark/results/`.
