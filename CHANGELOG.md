...
- A Claude Code hook row now states whether anything selects the hook file. Settings hooks are unchanged: `execute`, an expansion. A plugin manifest's hook file (`hooks/hooks.json` beside `.claude-plugin/plugin.json`, a `./` path in its `hooks`, or inline hooks) is read and kept `execute`/`high`, but is not an expansion, because plugin enablement is not in the repository. A `.claude/hooks/hooks.json` nothing selects is `access: unknown`, `risk: unknown`, still a row and never an expansion, so `diff`, check rows, the Stop hook and PR comments no longer present it as active authority. A hook file is no longer read as a settings file. A host-grants baseline that recorded such a file as `execute` reports one changed, non-widening row; no schema or contract version moves. (#714)
