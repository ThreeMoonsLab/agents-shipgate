...
5. **Then act.**
   - If `run_shipgate: true`: run `detect` through a **version-pinned,
     ...
     `uvx agents-shipgate@0.13.0 detect --workspace . --json` (or
     `pipx run agents-shipgate==0.13.0 ...`). Only fall back to a bare
     ...
     Action (`ThreeMoonsLab/agents-shipgate@v0.13.0`) if they prefer CI.
