...
1. **Run a scan and locate the top finding.**
   ...
   - **`insufficient_evidence` verdict** (v0.14+; the scan saw too many low-confidence tools or 4+ source warnings, an unproven binding graph, or an unresolved semantic/policy gap). There is no specific finding to fix. Check `release_decision.evidence_coverage.evidence_gaps[]` first: from v0.16 the action's `why` names the **selected** gap — the first row whose `next_action.path` is a non-empty string — and repeats that path, so open it and supply the reviewed declaration it asks for. Only when no row carries a path does the `why` fall back to "gather deeper sources" (MCP/OpenAPI inputs, eval traces, additional source files); that wording now means there genuinely is no surface to open. Either way this is a human declaration: never auto-write effect, authority, or binding claims.
