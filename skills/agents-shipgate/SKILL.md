...
2. For verifier runs, parse `agents-shipgate-reports/verifier.json` first:
   `merge_verdict` and `agent_controller`, then
   `agents-shipgate-reports/verify-run.json` for reproducibility. Then parse
...
- **CLI surface** is frozen for the current v1.0 alpha contract line — see https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/STABILITY.md.
...
| `verify_ref_error` | `verify` or `trigger` could not inspect the requested base/head refs. Fetch the refs locally and rerun. |
| `unknown_check_id` | The check ID passed to `explain` does not exist. Run `agents-shipgate list-checks --json` to enumerate. |
| `baseline_integrity_failure` | `baseline verify --strict` detected tampering or stale/expired baseline evidence. Inspect the baseline verifier output before regenerating. |
