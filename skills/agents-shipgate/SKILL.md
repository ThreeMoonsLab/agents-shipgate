...
- **Report JSON**: `report_schema_version: "0.9"`. Read `release_decision.decision` (`"blocked" | "review_required" | "passed"`) **first** for release gating — it is baseline-aware. Do not gate on `summary.status` for new consumers; it is preserved for v0.7 callers and is baseline-blind. Stable fields include `release_decision.{decision, reason, blockers, review_items, evidence_coverage, baseline_delta, fail_policy}`, `capability_facts[]` / `declared_intentions[]` / `misalignments[]` / `release_consequence` / `suggested_scenarios[]` (v0.9+), `summary.{critical_count, high_count, medium_count, status}` (legacy/compat), `manifest_dir` (top-level, v0.6+), `findings[].{id, fingerprint, check_id, severity, category, title, recommendation, suppressed}` plus v0.7 remediation fields `findings[].{autofix_safe, requires_human_review, suggested_patch_kind, docs_url}`. When scan ran with `--suggest-patches`, each active finding also carries `findings[].patches[]`. Reports validate against [`docs/report-schema.v0.9.json`](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/report-schema.v0.9.json) (current); older reports validate against [`docs/report-schema.v0.8.json`](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/report-schema.v0.8.json) and [`docs/report-schema.v0.7.json`](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/report-schema.v0.7.json) (frozen references).
- **Release Evidence Packet**: `agents-shipgate-reports/packet.{md,json,html}` (and `packet.pdf` with the `[pdf]` extras) is emitted alongside the report by default. The packet has ten always-present sections governed by [`docs/packet-schema.v0.1.json`](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/docs/packet-schema.v0.1.json) — see [STABILITY.md §Release Evidence Packet](https://github.com/ThreeMoonsLab/agents-shipgate/blob/main/STABILITY.md#release-evidence-packet-v01). Use the packet for reviewer-shaped output; use the report for finding details.
