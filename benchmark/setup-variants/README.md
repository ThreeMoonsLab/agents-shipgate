...
| [`25-codex-skill/`](25-codex-skill/) | Codex skill | Repo has `.agents/skills/agents-shipgate/` for repo-scoped Codex activation. |
...
The W2 baseline runs only `00-no-hints` and `10-agents-md` to keep the matrix small while establishing the discovery delta. Subsequent weeks expand to include `20`-`40`; Codex-specific runs use [`../matrix-codex.yaml`](../matrix-codex.yaml).
