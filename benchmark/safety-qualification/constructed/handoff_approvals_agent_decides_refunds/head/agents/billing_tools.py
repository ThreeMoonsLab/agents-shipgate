...
@function_tool(name_override="orders.decide_refund_request")
def decide_refund_request(request_id: str, decision: str, note: str = "") -> dict:
    """Approve or decline a pending refund request; an approved request is paid out by billing."""
    response = requests.post(
        f"{BILLING_API}/refund-requests/{request_id}/decision",
