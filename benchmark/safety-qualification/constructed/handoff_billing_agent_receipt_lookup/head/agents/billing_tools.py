...
@function_tool(name_override="orders.lookup_receipt")
def lookup_receipt(order_id: str) -> dict:
    """Fetch the receipt for an order: line items, total and settlement status."""
    response = requests.get(
        f"{BILLING_API}/orders/{order_id}/receipt", headers=_headers(), timeout=10
    )
    response.raise_for_status()
    return response.json()
