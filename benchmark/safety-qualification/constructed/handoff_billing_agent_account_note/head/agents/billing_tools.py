...
@function_tool(name_override="accounts.add_note")
def add_note(customer_id: str, note: str) -> dict:
    """Append a note to the account so the account team sees the context."""
    response = requests.post(
        f"{BILLING_API}/customers/{customer_id}/notes",
        json={"note": note},
        headers=_headers(),
        timeout=10,
    )
